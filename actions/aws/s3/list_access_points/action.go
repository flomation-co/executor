// Package aws_s3_list_access_points lists AWS S3 access points, optionally filtered by bucket.
package aws_s3_list_access_points
...
const (
	...
	Name         = "AWS S3 List Access Points"
	Description  = "List AWS S3 access points, optionally filtered by bucket."
...
var Inputs = [...]core.Connection{
	...
	{Name: "bucket", Type: core.ConnectionTypeString, Label: "Bucket Name (optional filter)", Placeholder: "my-bucket"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	in := &s3control.ListAccessPointsInput{AccountId: aws.String(accountID)}
	if bucket := awscommon.InputString("bucket", inputs); bucket != "" {
	...
	paginator := s3control.NewListAccessPointsPaginator(client, in)
	...
	return map[string]interface{}{
		"tool_result":   fmt.Sprintf("Found %d access point(s)", len(points)),
		"access_points": string(encoded),
		"count":         len(points),
	}, nil
}
