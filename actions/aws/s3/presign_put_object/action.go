// Package aws_s3_presign_put_object generates a time-limited presigned URL for
// uploading an object to S3.
package aws_s3_presign_put_object
...
const (
	...
	Name         = "AWS S3 Presign Put Object"
	Description  = "Generate a time-limited presigned URL for uploading an object to S3."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if contentType := awscommon.InputString("content_type", inputs); contentType != "" {
		putInput.ContentType = aws.String(contentType)
	}

	out, err := presigner.PresignPutObject(ctx, putInput, awsS3.WithPresignExpires(time.Duration(expiry)*time.Second))
	if err != nil {
		return nil, err
	}

	return map[string]interface{}{
		"tool_result":    fmt.Sprintf("Presigned PUT URL for %s/%s valid for %d second(s)", bucket, key, expiry),
		"url":            out.URL,
