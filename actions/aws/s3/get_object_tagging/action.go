// Package aws_s3_get_object_tagging reads the tag set of an S3 object.
package aws_s3_get_object_tagging
...
const (
	...
	Description  = "Read the tag set applied to an object in AWS S3."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	out, err := client.GetObjectTagging(ctx, &awsS3.GetObjectTaggingInput{Bucket: aws.String(bucket), Key: aws.String(key)})
	if err != nil {
		return nil, err
	}
	...
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Retrieved %d tag(s) for %s/%s", len(tagMap), bucket, key),
