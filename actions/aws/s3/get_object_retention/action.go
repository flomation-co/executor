// Package aws_s3_get_object_retention reads an object's WORM retention.
package aws_s3_get_object_retention
...
const (
	...
	Name         = "AWS S3 Get Object Retention"
	Description  = "Read the WORM retention mode and retain-until date of an AWS S3 object."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	in := &awsS3.GetObjectRetentionInput{
	...
	out, err := client.GetObjectRetention(ctx, in)
	if err != nil {
		return nil, err
	}

	var mode, retainUntil string
	if out.Retention != nil {
		mode = string(out.Retention.Mode)
		if out.Retention.RetainUntilDate != nil {
			retainUntil = out.Retention.RetainUntilDate.Format(time.RFC3339)
		}
	}

	return map[string]interface{}{
		"tool_result":  fmt.Sprintf("Retention on %s/%s: %s until %s", bucket, key, mode, retainUntil),
		"mode":         mode,
