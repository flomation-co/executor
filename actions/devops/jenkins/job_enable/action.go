package devops_jenkins_job_enable
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := jenkins.Post(cfg, jenkins.JobPath(job)+"/enable", "", nil)
	if err != nil {
		return jenkins.ErrorResult(err.Error()), nil
	}
	...
	return jenkins.SuccessResult(fmt.Sprintf("Enabled job %q", job), nil), nil
}
