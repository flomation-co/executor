package devops_jenkins_job_delete
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := jenkins.Post(cfg, jenkins.JobPath(job)+"/doDelete", "", nil)
	if err != nil {
		return jenkins.ErrorResult(err.Error()), nil
	}
	// doDelete answers 302 redirecting to the parent (followed to a 2xx).
	if err := jenkins.CheckResponse(resp, http.StatusFound); err != nil {
		return jenkins.ErrorResult(err.Error()), nil
	}
	return jenkins.SuccessResult(fmt.Sprintf("Deleted job %q", job), nil), nil
}
