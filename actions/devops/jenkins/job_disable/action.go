package devops_jenkins_job_disable
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := jenkins.Post(cfg, jenkins.JobPath(job)+"/disable", "", nil)
	if err != nil {
		return jenkins.ErrorResult(err.Error()), nil
	}
	...
	return jenkins.SuccessResult(fmt.Sprintf("Disabled job %q", job), nil), nil
}
