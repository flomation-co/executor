package devops_azuredevops_repo_get_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/repo_get"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestRepoGet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.HasSuffix(r.URL.Path, "/MyProject/_apis/git/repositories/MyRepo") {
			t.Errorf("path = %q", r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"id":"r1","name":"MyRepo","defaultBranch":"refs/heads/main"}`))
	}))
	defer srv.Close()
...
// TestRepoGetAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestRepoGetAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
