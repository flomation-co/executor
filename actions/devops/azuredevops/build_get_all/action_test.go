package devops_azuredevops_build_get_all_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/build_get_all"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestBuildGetAll(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		q := r.URL.Query()
		if q.Get("definitions") != "12,14" {
			t.Errorf("definitions = %q", q.Get("definitions"))
		}
		if q.Get("statusFilter") != "completed" || q.Get("resultFilter") != "failed" {
			t.Errorf("filters = %v", q)
		}
		if q.Get("branchName") != "refs/heads/main" {
			t.Errorf("branchName = %q, want the full ref", q.Get("branchName"))
		}
		_, _ = w.Write([]byte(`{"count":1,"value":[{"id":501,"result":"failed"}]}`))
	}))
	defer srv.Close()

	out, err := action.Execute(nil, nil, conns(srv.URL,
		&core.Connection{Name: "project", Type: core.ConnectionTypeString, Value: "MyProject"},
		&core.Connection{Name: "definition_ids", Type: core.ConnectionTypeString, Value: "12,14"},
		&core.Connection{Name: "status_filter", Type: core.ConnectionTypeString, Value: "completed"},
		&core.Connection{Name: "result_filter", Type: core.ConnectionTypeString, Value: "failed"},
		&core.Connection{Name: "branch_name", Type: core.ConnectionTypeString, Value: "main"},
	))
	if err != nil {
...
// TestBuildGetAllAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestBuildGetAllAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestBuildGetAllSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestBuildGetAllSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
