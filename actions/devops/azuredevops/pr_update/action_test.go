package devops_azuredevops_pr_update_test

import (
	"encoding/json"
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/pr_update"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestPrUpdate(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"pullRequestId":128,"status":"abandoned"}`))
	}))
	defer srv.Close()
	...
	out, err := action.Execute(nil, nil, conns(srv.URL,
		...
		&core.Connection{Name: "status", Type: core.ConnectionTypeString, Value: "abandoned"},
	))
	if err != nil {
	...
	if out["pr_status"] != "abandoned" {
		t.Errorf("pr_status = %v", out["pr_status"])
	}
}

// TestPrUpdateAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestPrUpdateAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestPrUpdateSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestPrUpdateSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
