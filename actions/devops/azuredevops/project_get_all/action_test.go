package devops_azuredevops_project_get_all_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/project_get_all"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestProjectGetAll(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.URL.Query().Get("stateFilter"); got != "all" {
			t.Errorf("stateFilter = %q, want all", got)
		}
		_, _ = w.Write([]byte(`{"count":2,"value":[{"id":"p1","name":"MyProject"},{"id":"p2","name":"Other"}]}`))
	}))
	defer srv.Close()

	out, err := action.Execute(nil, nil, conns(srv.URL,
		&core.Connection{Name: "state_filter", Type: core.ConnectionTypeString, Value: "all"},
...
// TestProjectGetAllAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestProjectGetAllAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestProjectGetAllSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestProjectGetAllSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
