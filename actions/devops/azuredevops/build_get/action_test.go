package devops_azuredevops_build_get_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/build_get"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestBuildGet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.HasSuffix(r.URL.Path, "/_apis/build/builds/501") {
			t.Errorf("path = %q", r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"id":501,"status":"completed","result":"succeeded"}`))
	}))
	defer srv.Close()
	...
	if out["build_status"] != "completed" || out["build_result"] != "succeeded" {
		t.Errorf("status/result = %v/%v", out["build_status"], out["build_result"])
	}
}

// TestBuildGetAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestBuildGetAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestBuildGetSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestBuildGetSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
