package devops_azuredevops_pr_get_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/pr_get"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestPrGet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"pullRequestId":128,"status":"active","mergeStatus":"succeeded"}`))
	}))
	defer srv.Close()
	...
	if out["pr_status"] != "active" || out["merge_status"] != "succeeded" {
		t.Errorf("status/mergeStatus = %v/%v", out["pr_status"], out["merge_status"])
	}
	if out["id"] != "128" {
...
// TestPrGetAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestPrGetAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
