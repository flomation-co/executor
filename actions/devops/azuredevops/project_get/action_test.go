package devops_azuredevops_project_get_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/project_get"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestProjectGet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.HasSuffix(r.URL.Path, "/_apis/projects/MyProject") {
			t.Errorf("path = %q, want the project appended", r.URL.Path)
		}
		if r.URL.Query().Get("includeCapabilities") != "true" {
			t.Error("includeCapabilities was not sent")
		}
		_, _ = w.Write([]byte(`{"id":"p1","name":"MyProject","state":"wellFormed"}`))
	}))
	defer srv.Close()

	out, err := action.Execute(nil, nil, conns(srv.URL,
		&core.Connection{Name: "project", Type: core.ConnectionTypeString, Value: "MyProject"},
		&core.Connection{Name: "include_capabilities", Type: core.ConnectionTypeBoolean, Value: true},
	))
	if err != nil {
	...
	if out["id"] != "p1" {
		t.Errorf("id = %v, want p1", out["id"])
	}
}

// TestProjectGetAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestProjectGetAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestProjectGetSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestProjectGetSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
