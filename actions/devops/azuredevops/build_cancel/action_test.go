package devops_azuredevops_build_cancel_test

import (
	"encoding/json"
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/build_cancel"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestBuildCancel(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["status"] != "cancelling" {
			t.Errorf("body = %v, want status cancelling", body)
		}
		_, _ = w.Write([]byte(`{"id":501,"status":"cancelling"}`))
	}))
	defer srv.Close()
	...
	if out["build_status"] != "cancelling" {
		t.Errorf("build_status = %v", out["build_status"])
	}
	if out["id"] != "501" {
		t.Errorf("id = %v, want 501", out["id"])
	}
}

// TestBuildCancelAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestBuildCancelAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestBuildCancelSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestBuildCancelSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
