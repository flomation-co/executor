package devops_azuredevops_pr_get_all_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/pr_get_all"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestPrGetAll(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if q.Get("searchCriteria.status") != "active" {
			t.Errorf("status = %q", q.Get("searchCriteria.status"))
		}
		if q.Get("searchCriteria.targetRefName") != "refs/heads/main" {
			t.Errorf("targetRefName = %q, want the full ref", q.Get("searchCriteria.targetRefName"))
		}
		if q.Get("searchCriteria.reviewerId") != "id-1" {
			t.Errorf("reviewerId = %q", q.Get("searchCriteria.reviewerId"))
		}
		_, _ = w.Write([]byte(`{"count":1,"value":[{"pullRequestId":128}]}`))
	}))
	defer srv.Close()
	...
	out, err := action.Execute(nil, nil, conns(srv.URL,
		...
		&core.Connection{Name: "status", Type: core.ConnectionTypeString, Value: "active"},
		&core.Connection{Name: "target_branch", Type: core.ConnectionTypeString, Value: "main"},
		&core.Connection{Name: "reviewer_id", Type: core.ConnectionTypeString, Value: "id-1"},
	))
	if err != nil {
...
// TestPrGetAllAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestPrGetAllAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestPrGetAllSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestPrGetAllSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
