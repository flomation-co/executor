package devops_azuredevops_pr_complete_test

import (
	"encoding/json"
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/pr_complete"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestPrComplete(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet {
			// The read that supplies lastMergeSourceCommit.
			_, _ = w.Write([]byte(`{"pullRequestId":128,"status":"active","lastMergeSourceCommit":{"commitId":"abc123"}}`))
			return
		}
		...
		// Echoing the commit back is the concurrency guard; omit it and Azure
		// DevOps refuses the merge.
		commit := body["lastMergeSourceCommit"].(map[string]interface{})
		if commit["commitId"] != "abc123" {
			t.Errorf("lastMergeSourceCommit = %v, want the one read from the PR", body["lastMergeSourceCommit"])
		}
		opts := body["completionOptions"].(map[string]interface{})
		if opts["mergeStrategy"] != "squash" || opts["deleteSourceBranch"] != true {
			t.Errorf("completionOptions = %v", opts)
		}
		_, _ = w.Write([]byte(`{"pullRequestId":128,"status":"completed","mergeStatus":"succeeded"}`))
	}))
	defer srv.Close()
	...
	out, err := action.Execute(nil, nil, conns(srv.URL,
		...
		&core.Connection{Name: "merge_strategy", Type: core.ConnectionTypeString, Value: "squash"},
		&core.Connection{Name: "delete_source_branch", Type: core.ConnectionTypeBoolean, Value: true},
	))
	if err != nil {
	...
	if out["pr_status"] != "completed" {
		t.Errorf("pr_status = %v", out["pr_status"])
	}
}

// TestPrCompleteAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestPrCompleteAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestPrCompleteSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestPrCompleteSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
