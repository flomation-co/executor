package devops_azuredevops_branch_get_all_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/branch_get_all"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestBranchGetAll(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.URL.Query().Get("filter"); got != "heads/release/" {
			t.Errorf("filter = %q, want heads/ prefixed", got)
		}
		_, _ = w.Write([]byte(`{"count":1,"value":[{"name":"refs/heads/release/2.0"}]}`))
	}))
	defer srv.Close()
	...
	out, err := action.Execute(nil, nil, conns(srv.URL,
		...
		&core.Connection{Name: "filter", Type: core.ConnectionTypeString, Value: "release/"},
	))
	if err != nil {
...
// TestBranchGetAllAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestBranchGetAllAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestBranchGetAllSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestBranchGetAllSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
