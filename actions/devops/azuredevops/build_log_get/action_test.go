package devops_azuredevops_build_log_get_test

import (
	core "flomation.app/automate/executor"
	action "flomation.app/automate/executor/actions/devops/azuredevops/build_log_get"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
...
func TestBuildLogGet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.HasSuffix(r.URL.Path, "/_apis/build/builds/501/logs/3") {
			t.Errorf("path = %q", r.URL.Path)
		}
		w.Header().Set("Content-Type", "text/plain")
		_, _ = w.Write([]byte("Starting: Build\nDone\n"))
	}))
	defer srv.Close()

	out, err := action.Execute(nil, nil, conns(srv.URL,
		&core.Connection{Name: "project", Type: core.ConnectionTypeString, Value: "MyProject"},
		&core.Connection{Name: "build_id", Type: core.ConnectionTypeInteger, Value: 501},
		&core.Connection{Name: "log_id", Type: core.ConnectionTypeInteger, Value: 3},
	))
	if err != nil {
	...
	if !strings.Contains(out["content"].(string), "Starting: Build") {
		t.Errorf("content = %q", out["content"])
	}
	if out["truncated"] != false {
		t.Error("a short log must not be reported as truncated")
	}
}

// TestBuildLogGetAPIError pins the soft-failure contract: a provider error is data on
// the error port, never a hard Go error that would abort the flow run.
func TestBuildLogGetAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestBuildLogGetSignInPage pins the 203 trap: a bad or expired PAT answers 203 with an
// HTML sign-in page rather than 401, so a plain 2xx check would call this a
// success and then fail on an HTML-to-JSON decode.
func TestBuildLogGetSignInPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
