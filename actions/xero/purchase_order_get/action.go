...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := xero_common.DoJSON(flow, "GET", "/PurchaseOrders/"+id, token, tenant, nil)
	if err != nil {
		return xero_common.MapError(err), nil
	}

	obj := xero_common.FirstElement(resp, "PurchaseOrders")
	return xero_common.ObjectResult(id, obj, fmt.Sprintf("Fetched Xero purchase order %s", id)), nil
}
