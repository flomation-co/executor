...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	inv, err := stripe_common.NewClient(apiKey).Invoices.VoidInvoice(id, &stripe.InvoiceVoidInvoiceParams{})
	if err != nil {
		return stripe_common.MapError(err), nil
	}
	return stripe_common.ObjectResult(inv, fmt.Sprintf("Voided invoice %s", inv.ID)), nil
}
