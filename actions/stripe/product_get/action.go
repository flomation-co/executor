...
const (
	...
	Summary      = "Look up a product in Stripe"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	prod, err := stripe_common.NewClient(apiKey).Products.Get(id, params)
	if err != nil {
		return stripe_common.MapError(err), nil
	}
	return stripe_common.ObjectResult(prod, fmt.Sprintf("Retrieved product %s", prod.ID)), nil
}
