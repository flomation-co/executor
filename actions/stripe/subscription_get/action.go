...
const (
	...
	Summary      = "Look up a subscription in Stripe"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	sub, err := stripe_common.NewClient(apiKey).Subscriptions.Get(id, params)
	if err != nil {
		return stripe_common.MapError(err), nil
	}
	return stripe_common.ObjectResult(sub, fmt.Sprintf("Retrieved subscription %s", sub.ID)), nil
}
