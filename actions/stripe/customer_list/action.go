...
const (
	...
	Summary      = "List customers in Stripe"
...
var Inputs = [...]core.Connection{
	...
	{Name: "email", Type: core.ConnectionTypeString, Label: "Email", Placeholder: "Filter by exact email (optional)"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	it := stripe_common.NewClient(apiKey).Customers.List(params)
	var items []map[string]interface{}
	for it.Next() {
		items = append(items, stripe_common.ToMap(it.Customer()))
	}
	if err := it.Err(); err != nil {
		return stripe_common.MapError(err), nil
	}
	hasMore := it.Meta() != nil && it.Meta().HasMore
	return stripe_common.ListResult(items, hasMore, fmt.Sprintf("Listed %d customer(s)", len(items))), nil
}
