...
const (
	...
	Summary      = "List charges in Stripe"
...
var Outputs = [...]core.Connection{
	...
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Charges"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	it := stripe_common.NewClient(apiKey).Charges.List(params)
	var items []map[string]interface{}
	for it.Next() {
		items = append(items, stripe_common.ToMap(it.Charge()))
	}
	if err := it.Err(); err != nil {
		return stripe_common.MapError(err), nil
	}
	hasMore := it.Meta() != nil && it.Meta().HasMore
	return stripe_common.ListResult(items, hasMore, fmt.Sprintf("Listed %d charge(s)", len(items))), nil
}
