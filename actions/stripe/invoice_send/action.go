...
const (
	...
	Name         = "Invoice: Send"
	Description  = "Manually send a Stripe invoice to the customer by email."
	Summary      = "Send an invoice in Stripe"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	inv, err := stripe_common.NewClient(apiKey).Invoices.SendInvoice(id, &stripe.InvoiceSendInvoiceParams{})
	if err != nil {
		return stripe_common.MapError(err), nil
	}
	return stripe_common.ObjectResult(inv, fmt.Sprintf("Sent invoice %s", inv.ID)), nil
}
