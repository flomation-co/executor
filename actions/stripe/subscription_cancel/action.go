...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	sub, err := stripe_common.NewClient(apiKey).Subscriptions.Cancel(id, &stripe.SubscriptionCancelParams{})
	if err != nil {
		return stripe_common.MapError(err), nil
	}
	return stripe_common.ObjectResult(sub, fmt.Sprintf("Cancelled subscription %s", sub.ID)), nil
}
