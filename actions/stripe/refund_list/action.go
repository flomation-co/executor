...
const (
	...
	Name         = "Refund: List"
	Description  = "List Stripe refunds, optionally filtered by charge or payment intent."
	Summary      = "List refunds in Stripe"
...
var Inputs = [...]core.Connection{
	...
	{Name: "charge", Type: core.ConnectionTypeString, Label: "Charge", Placeholder: "Filter by charge ID (optional)"},
	{Name: "payment_intent", Type: core.ConnectionTypeString, Label: "Payment Intent", Placeholder: "Filter by payment intent ID (optional)"},
	{Name: "limit", Type: core.ConnectionTypeInteger, Label: "Limit", Placeholder: "Max results (default 20, max 100)"},
}

var Outputs = [...]core.Connection{
	{Name: "tool_result", Type: core.ConnectionTypeString, Label: "Result Summary"},
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Refunds"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	it := stripe_common.NewClient(apiKey).Refunds.List(params)
	var items []map[string]interface{}
	for it.Next() {
		items = append(items, stripe_common.ToMap(it.Refund()))
	}
	if err := it.Err(); err != nil {
		return stripe_common.MapError(err), nil
	}
	hasMore := it.Meta() != nil && it.Meta().HasMore
	return stripe_common.ListResult(items, hasMore, fmt.Sprintf("Listed %d refund(s)", len(items))), nil
}
