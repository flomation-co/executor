...
const (
	...
	Name         = "Payout: List"
	Description  = "List Stripe payouts, optionally filtered by status."
	Summary      = "List payouts in Stripe"
...
var Inputs = [...]core.Connection{
	...
	{Name: "status", Type: core.ConnectionTypeString, Label: "Status", Placeholder: "Filter by status: pending, paid, failed, canceled (optional)"},
	{Name: "limit", Type: core.ConnectionTypeInteger, Label: "Limit", Placeholder: "Max results (default 20, max 100)"},
}

var Outputs = [...]core.Connection{
	{Name: "tool_result", Type: core.ConnectionTypeString, Label: "Result Summary"},
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Payouts"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	it := stripe_common.NewClient(apiKey).Payouts.List(params)
	var items []map[string]interface{}
	for it.Next() {
		items = append(items, stripe_common.ToMap(it.Payout()))
	}
	if err := it.Err(); err != nil {
		return stripe_common.MapError(err), nil
	}
	hasMore := it.Meta() != nil && it.Meta().HasMore
	return stripe_common.ListResult(items, hasMore, fmt.Sprintf("Listed %d payout(s)", len(items))), nil
}
