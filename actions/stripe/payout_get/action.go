...
const (
	...
	Summary      = "Look up a payout in Stripe"
...
var Inputs = [...]core.Connection{
	...
	{Name: "payout_id", Type: core.ConnectionTypeString, Label: "Payout ID", Placeholder: "po_…", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	id, err := stripe_common.RequiredString("payout_id", inputs)
	if err != nil {
		return stripe_common.ErrorResult(err.Error()), nil
	}

	payout, err := stripe_common.NewClient(apiKey).Payouts.Get(id, &stripe.PayoutParams{})
	if err != nil {
		return stripe_common.MapError(err), nil
	}
	return stripe_common.ObjectResult(payout, fmt.Sprintf("Retrieved payout %s", payout.ID)), nil
}
