...
const (
	...
	Summary      = "Look up a customer in Stripe"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	cust, err := stripe_common.NewClient(apiKey).Customers.Get(id, params)
	if err != nil {
		return stripe_common.MapError(err), nil
	}
	return stripe_common.ObjectResult(cust, fmt.Sprintf("Retrieved customer %s", cust.ID)), nil
}
