...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	pm, err := stripe_common.NewClient(apiKey).PaymentMethods.Detach(id, &stripe.PaymentMethodDetachParams{})
	if err != nil {
		return stripe_common.MapError(err), nil
	}
	return stripe_common.ObjectResult(pm, fmt.Sprintf("Detached payment method %s", pm.ID)), nil
}
