package github_cancel_workflow_run
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := github.RepoAPI(token, baseURL, owner, repo, "POST", fmt.Sprintf("/actions/runs/%s/cancel", runID), nil)
	if err != nil {
		return github.ErrorResult(fmt.Sprintf("Failed to cancel workflow run: %s", err)), nil
	}
	...
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Cancelled workflow run %s", runID),
		"success":     true,
		"error":       "",
	}, nil
}
