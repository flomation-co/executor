// Package product_get implements the Freshsales "Product: Get" action.
package product_get
...
const (
	...
	Name         = "Product: Get"
	Description  = "Fetch a Freshsales CPQ product by ID."
	Summary      = "Look up a product in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/cpq/products/"+idValue, nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}
	...
	return freshsales_common.ObjectResult(recordOut, fmt.Sprintf("Fetched product %s", freshsales_common.NameOf(recordOut))), nil
}
