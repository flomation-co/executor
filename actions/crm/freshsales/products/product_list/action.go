// Package product_list implements the Freshsales "Product: List" action.
...
const (
	...
	Name         = "Product: List"
	Description  = "List Freshsales CPQ products."
	Summary      = "List products in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/cpq/products", nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	items := freshsales_common.Arr(resp, "products")
	return freshsales_common.ListResult(items, fmt.Sprintf("Products: %d record(s)", len(items))), nil
}
