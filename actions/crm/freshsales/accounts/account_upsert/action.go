// Package account_upsert implements the Freshsales "Account: Upsert" action.
package account_upsert
...
const (
	...
	Name         = "Account: Upsert"
	Description  = "Create or update a account matched on a unique identifier."
	Summary      = "Add or update an account in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodPost, "/sales_accounts/upsert", payload, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}
	...
	return freshsales_common.ObjectResult(recordOut, fmt.Sprintf("Upserted account %s", freshsales_common.NameOf(recordOut))), nil
}
