// Package account_list_by_view implements the Freshsales "Account: List By View" action.
package account_list_by_view
...
const (
	...
	Name         = "Account: List By View"
	Description  = "List sales_accounts in a saved view. Get the view ID from Account: List Views."
	Summary      = "List the companies in a saved view"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/sales_accounts/view/"+view_idValue, nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	items := freshsales_common.Arr(resp, "sales_accounts")
	return freshsales_common.ListResult(items, fmt.Sprintf("Account list: %d record(s)", len(items))), nil
}
