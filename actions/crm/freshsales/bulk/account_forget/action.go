// Package account_forget implements the Freshsales "GDPR: Forget Account" action.
package account_forget
...
const (
	...
	Name         = "GDPR: Forget Account"
	Description  = "Permanently erase a account and its data for a GDPR request. Cannot be undone."
	Summary      = "Erase a company for a data request"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	_, err = client.Do(flow, http.MethodDelete, "/sales_accounts/"+idValue+"/forget", nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	return freshsales_common.OkResult("Erased account"), nil
}
