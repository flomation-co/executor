// Package deal_bulk_upsert implements the Freshsales "Bulk: Upsert Deals" action.
package deal_bulk_upsert
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	payload := map[string]interface{}{"deals": records}
	...
	resp, err := client.Do(flow, http.MethodPost, "/deals/bulk_upsert", payload, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	return freshsales_common.PlainResult(resp, "Bulk upsert deals"), nil
}
