// Package contact_forget implements the Freshsales "GDPR: Forget Contact" action.
package contact_forget
...
const (
	...
	Name         = "GDPR: Forget Contact"
	Description  = "Permanently erase a contact and its data for a GDPR request. Cannot be undone."
	Summary      = "Erase a contact for a data request"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	_, err = client.Do(flow, http.MethodDelete, "/contacts/"+idValue+"/forget", nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	return freshsales_common.OkResult("Erased contact"), nil
}
