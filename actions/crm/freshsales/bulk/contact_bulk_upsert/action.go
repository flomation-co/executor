// Package contact_bulk_upsert implements the Freshsales "Bulk: Upsert Contacts" action.
package contact_bulk_upsert
...
const (
	...
	Description  = "Create or update many contacts in one call. Cheaper against the hourly rate limit."
	Summary      = "Add or update many contacts at once"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	payload := map[string]interface{}{"contacts": records}
	...
	resp, err := client.Do(flow, http.MethodPost, "/contacts/bulk_upsert", payload, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	return freshsales_common.PlainResult(resp, "Bulk upsert contacts"), nil
}
