// Package deal_clone implements the Freshsales "Deal: Clone" action.
package deal_clone
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodPost, "/deals/"+idValue+"/clone", payload, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}
	...
	return freshsales_common.ObjectResult(recordOut, fmt.Sprintf("Cloned deal %s", freshsales_common.NameOf(recordOut))), nil
}
