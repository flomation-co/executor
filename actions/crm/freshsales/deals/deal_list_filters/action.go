// Package deal_list_filters implements the Freshsales "Deal: List Views" action.
package deal_list_filters
...
const (
	...
	Name         = "Deal: List Views"
	Description  = "List the saved views (filters) available for deals. Use a view ID with List By View."
	Summary      = "List the saved views for deals"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/deals/filters", nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	items := freshsales_common.Arr(resp, "filters")
	return freshsales_common.ListResult(items, fmt.Sprintf("Deal views: %d record(s)", len(items))), nil
}
