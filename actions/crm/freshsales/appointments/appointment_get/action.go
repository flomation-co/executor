// Package appointment_get implements the Freshsales "Appointment: Get" action.
package appointment_get
...
const (
	...
	Description  = "Fetch a single Freshsales appointment by ID."
	Summary      = "Look up an appointment in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/appointments/"+idValue, nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}
	...
	return freshsales_common.ObjectResult(recordOut, fmt.Sprintf("Fetched appointment %s", freshsales_common.NameOf(recordOut))), nil
}
