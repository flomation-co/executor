// Package appointment_list implements the Freshsales "Appointment: List" action.
package appointment_list
...
const (
	...
	Description  = "List Freshsales appointments with optional filtering and paging."
	Summary      = "List appointments in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/appointments", nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	items := freshsales_common.Arr(resp, "appointments")
	return freshsales_common.ListResult(items, fmt.Sprintf("Appointment list: %d record(s)", len(items))), nil
}
