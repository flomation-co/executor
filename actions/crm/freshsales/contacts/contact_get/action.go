// Package contact_get implements the Freshsales "Contact: Get" action.
package contact_get
...
const (
	...
	Name         = "Contact: Get"
	Description  = "Fetch a single Freshsales contact by ID."
	Summary      = "Look up a contact in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/contacts/"+idValue, nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}
	...
	return freshsales_common.ObjectResult(recordOut, fmt.Sprintf("Fetched contact %s", freshsales_common.NameOf(recordOut))), nil
}
