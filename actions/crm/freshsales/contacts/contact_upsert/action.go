// Package contact_upsert implements the Freshsales "Contact: Upsert" action.
package contact_upsert
...
const (
	...
	Name         = "Contact: Upsert"
	Description  = "Create or update a contact matched on a unique identifier."
	Summary      = "Add or update a contact in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodPost, "/contacts/upsert", payload, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}
	...
	return freshsales_common.ObjectResult(recordOut, fmt.Sprintf("Upserted contact %s", freshsales_common.NameOf(recordOut))), nil
}
