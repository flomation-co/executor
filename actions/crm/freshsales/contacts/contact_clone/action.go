// Package contact_clone implements the Freshsales "Contact: Clone" action.
package contact_clone
...
const (
	...
	Name         = "Contact: Clone"
	Description  = "Duplicate an existing Freshsales contact."
	Summary      = "Duplicate a contact in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodPost, "/contacts/"+idValue+"/clone", payload, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}
	...
	return freshsales_common.ObjectResult(recordOut, fmt.Sprintf("Cloned contact %s", freshsales_common.NameOf(recordOut))), nil
}
