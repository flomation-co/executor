// Package list_remove_contacts implements the Freshsales "List: Remove Contacts" action.
package list_remove_contacts
...
const (
	...
	Description  = "Remove contacts from a Freshsales marketing list."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodPost, "/lists/"+idValue+"/remove_contacts", payload, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	return freshsales_common.PlainResult(resp, "Removed contacts from list"), nil
}
