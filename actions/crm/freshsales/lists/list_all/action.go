// Package list_all implements the Freshsales "List: List All" action.
package list_all
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/lists", nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	items := freshsales_common.Arr(resp, "lists")
	return freshsales_common.ListResult(items, fmt.Sprintf("Marketing lists: %d record(s)", len(items))), nil
}
