// Package sales_activity_list_fields implements the Freshsales "Sales Activity: List Fields" action.
package sales_activity_list_fields
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/settings/sales_activities/fields", nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	items := freshsales_common.Arr(resp, "fields")
	return freshsales_common.ListResult(items, fmt.Sprintf("Sales activity fields: %d record(s)", len(items))), nil
}
