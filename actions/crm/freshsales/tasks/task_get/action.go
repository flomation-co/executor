// Package task_get implements the Freshsales "Task: Get" action.
package task_get
...
const (
	...
	Name         = "Task: Get"
	Description  = "Fetch a single Freshsales task by ID."
	Summary      = "Look up a task in Freshsales"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/tasks/"+idValue, nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}
	...
	return freshsales_common.ObjectResult(recordOut, fmt.Sprintf("Fetched task %s", freshsales_common.NameOf(recordOut))), nil
}
