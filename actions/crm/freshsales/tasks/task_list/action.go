// Package task_list implements the Freshsales "Task: List" action.
package task_list
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.Do(flow, http.MethodGet, "/tasks", nil, query)
	if err != nil {
		return freshsales_common.ErrorResult(err.Error()), nil
	}

	items := freshsales_common.Arr(resp, "tasks")
	return freshsales_common.ListResult(items, fmt.Sprintf("Task list: %d record(s)", len(items))), nil
}
