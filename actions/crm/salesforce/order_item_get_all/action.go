package crm_salesforce_order_item_get_all
...
//
// It deliberately does NOT include Name or ProductCode. OrderItem has neither
// field — verified against the live org's describe, and "SELECT Id, Name FROM
// OrderItem" is a hard INVALID_FIELD. That trap is why the product's name is read
// through the relationship (Product2.Name) instead, which does work.
// OrderItemNumber is Salesforce's own line reference and is what an operator
// matches against a paper order.
const defaultFields = "Id,OrderItemNumber,OrderId,Product2Id,Product2.Name,PricebookEntryId,Quantity,UnitPrice,ListPrice,TotalPrice,ServiceDate,EndDate,Description,AvailableQuantity"
...
var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Order Lines"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// There is no REST "list the children of this record" call for order lines, so
	// this is a SOQL query — built through BuildQuery so the order ID is escaped
	// and quoted rather than pasted into a string.
	soql, err := salesforce.BuildQuery(
		"OrderItem",
		fields,
		[]salesforce.Condition{{Field: "OrderId", Operator: "=", Value: orderID}},
		false,
	...
	if returnAll && nextURL != "" && pages >= salesforce.MaxAllPages {
		out["tool_result"] = fmt.Sprintf("Fetched %d product line(s) on order %s across %d page(s); stopped at the %d-page safety cap", len(records), orderID, pages, salesforce.MaxAllPages)
	} else {
		out["tool_result"] = fmt.Sprintf("Found %d product line(s) on order %s%s", len(records), orderID, salesforce.TruncationHint(len(records), limit, returnAll))
	}
	return out, nil
}
