...
var Inputs = [...]core.Connection{
	...
	{Name: "filter_value", Type: core.ConnectionTypeString, Label: "Filter Value", Placeholder: "Renewal call — for Contains use % as the wildcard (%renewal%); or a date shortcut like TODAY or LAST_N_DAYS:7"},
	{Name: "filter_conditions", Type: core.ConnectionTypeObject, Label: "More Filters", Placeholder: `[{"field":"IsClosed","operator":"=","value":"false"}]`},
	{Name: "match_any_filter", Type: core.ConnectionTypeBoolean, Label: "Match ANY filter instead of all", Placeholder: "On, matching any one of the filters is enough. The Due On or After / Due On or Before dates always apply"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// becomes a Condition and goes through the shared query builder, which is the
	// one place identifiers are whitelisted and values are escaped — and which
	// asks Salesforce what each field actually is, so a number compares as a
	// number. Nothing here builds SOQL by hand.
	...
	filters = appendEqualsFilter(filters, "Status", salesforce.OptionalString("task_status", inputs))
	filters = appendEqualsFilter(filters, "Priority", salesforce.OptionalString("task_priority", inputs))
	...
	// The date range is SCOPE, not a filter: the two boxes read as one control
	// bounding the period, and "Match ANY filter instead of all" must not reach
	// them. ORed, `ActivityDate >= 1 July OR ActivityDate <= 31 July` is true of
	// every task in the org — NULL due dates included — so the range silently
	// became "everything" and took every other filter with it. Live: 6 rows
	// ANDed, 33 (the whole table) ORed.
	//
	...
	if from := dateOnly(salesforce.OptionalString("due_from", inputs)); from != "" {
		scope = append(scope, salesforce.Condition{Field: "ActivityDate", Operator: ">=", Value: from})
	}
	if to := dateOnly(salesforce.OptionalString("due_to", inputs)); to != "" {
		scope = append(scope, salesforce.Condition{Field: "ActivityDate", Operator: "<=", Value: to})
	}
	...
	default:
		...
		out["tool_result"] = fmt.Sprintf("Found %d task(s)%s", len(records), salesforce.TruncationHint(len(records), limit, returnAll))
	}
	return out, nil
}
