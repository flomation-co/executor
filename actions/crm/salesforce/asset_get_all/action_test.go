...
// was actually sent — so a test can prove that a bad ID never reached the org.
...
func (rec *recorder) serve(t *testing.T) func() {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case strings.HasSuffix(r.URL.Path, "/sobjects/Asset/describe"):
			_ = json.NewEncoder(w).Encode(map[string]interface{}{
				"name": "Asset",
				"fields": []map[string]interface{}{
					{"name": "AccountId", "type": "reference"},
					{"name": "ContactId", "type": "reference"},
...
// TestALookupBoxGivenANameIsNamedRatherThanDumpingSOQL is the regression test.
// An upstream step that resolved to a company NAME used to have that name dropped
// straight into "AccountId = '...'", and Salesforce answered with a fragment of the
// SOQL, a caret, a row/column position and INVALID_QUERY_FILTER_OPERATOR — a code
// that reads as though the operator's own Filter Comparison were at fault.
func TestALookupBoxGivenANameIsNamedRatherThanDumpingSOQL(t *testing.T) {
	for _, tc := range []struct {
		...
		{"contact_id", "priya@example.com", "Customer (Contact)"},
		{"product_id", "GenWatt Diesel 200kW", "Product"},
	} {
