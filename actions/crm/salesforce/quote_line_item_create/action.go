...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if entryID != "" {
		...
		entryID, entryBook, listPrice, havePrice, err = entryForProduct(instanceURL, token, productID, target)
	}
	if err != nil {
		return salesforce.ErrorResult(err.Error()), nil
	}
	...
	if pinned {
		summary += fmt.Sprintf(" — the quote had no price book, so it was put on %s to match the product", entryBook)
	}
