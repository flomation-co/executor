...
const (
	...
	Name         = "Account: Update"
	Description  = "Update an existing Apollo account by ID. Only supplied fields change."
	Summary      = "Change an account in Apollo"
...
var Inputs = [...]core.Connection{
	...
	{Name: "account_id", Type: core.ConnectionTypeString, Label: "Account ID", Placeholder: "The Apollo account ID to update", Required: true},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := apollo_common.NewClient(apiKey).Patch(flow, "/accounts/"+id, body)
	if err != nil {
	...
	if account == nil {
		return apollo_common.ErrorResult("account was not updated"), nil
	}
	return apollo_common.ObjectResult(id, account, fmt.Sprintf("Updated account %s", id)), nil
}
