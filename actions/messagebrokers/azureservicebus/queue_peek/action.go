package messagebrokers_azureservicebus_queue_peek
...
const (
	...
	Name         = "Azure Service Bus: Peek Queue"
	Description  = "Look at a queue's messages without locking, removing or counting a delivery against them. The safe first question — nothing observed here is consumed. Peek also sees scheduled and deferred messages, which a receive cannot."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	spec := sb.ReceiverSpec{Queue: queue}
	...
	if err != nil {
		return sb.Fail(auth, fmt.Sprintf("Could not peek queue %s", spec.Entity()), err), nil
	}
	return sb.MessagesResult(sb.MessagesOutput(msgs, sb.OptionalBool("parse_json", inputs)), "Peeked", "queue "+spec.Entity()), nil
}
