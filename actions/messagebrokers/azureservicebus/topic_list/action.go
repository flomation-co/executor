package messagebrokers_azureservicebus_topic_list
...
const (
	...
	Name         = "Azure Service Bus: Get Many Topics"
	Description  = "List the topics in the namespace with their configuration. Needs a namespace-level connection string or an Entra principal."
...
var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Topics"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, err := client.ListTopics(ctx, sb.ClampLimit(inputs))
	if err != nil {
		return sb.Fail(auth, "Could not list topics", err), nil
	}
	out := make([]interface{}, 0, len(items))
	for _, t := range items {
		out = append(out, sb.TopicOutput(t.TopicName, t.TopicProperties))
	}
	return sb.ListResult(out, sb.ListSummary("topic", len(out))), nil
}
