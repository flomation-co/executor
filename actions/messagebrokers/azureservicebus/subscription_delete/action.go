package messagebrokers_azureservicebus_subscription_delete
...
const (
	...
	Name         = "Azure Service Bus: Delete Subscription"
	Description  = "Delete a subscription and every message on it. The topic and its other subscriptions are untouched."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if err := client.DeleteSubscription(ctx, topic, subscription); err != nil {
		return sb.Fail(auth, fmt.Sprintf("Could not delete subscription %s on topic %s", subscription, topic), err), nil
	}
	return sb.ResourceResult(subscription, map[string]interface{}{"name": subscription, "topic": topic, "deleted": true},
		fmt.Sprintf("Deleted subscription %s on topic %s", subscription, topic)), nil
}
