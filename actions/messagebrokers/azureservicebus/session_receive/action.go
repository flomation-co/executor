package messagebrokers_azureservicebus_session_receive
...
var Inputs = [...]core.Connection{
	...
	{Name: "session_id", Type: core.ConnectionTypeString, Label: "Session ID", Placeholder: "Blank to accept the next available session — waits until one is free, then times out"},
	...
	{Name: "session_state", Type: core.ConnectionTypeText, Label: "Set Session State", Placeholder: "Written back to the session after the messages are settled; leave blank to leave it alone"},
...
var Outputs = [...]core.Connection{
	...
	{Name: "session_state", Type: core.ConnectionTypeString, Label: "Session State"},
	{Name: "locked_until", Type: core.ConnectionTypeString, Label: "Session Locked Until"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	ctx := sb.Context(flow)
	// Accepting the next available session BLOCKS until one is free, so the
	// accept shares the Max Wait budget rather than running unbounded.
	acceptCtx, cancelAccept := context.WithTimeout(ctx, sb.ClampMaxWait(inputs))
	defer cancelAccept()

	receiver, err := sb.NewSessionReceiver(acceptCtx, auth, spec, sb.OptionalString("session_id", inputs))
	if err != nil {
		return sb.Fail(auth, fmt.Sprintf("Could not accept a session on %s (a session receive needs an entity created with Requires Session — the flag cannot be added later)", spec.Entity()), err), nil
	}
	defer func() { _ = receiver.Close(ctx) }()

	msgs, err := sb.ReceiveWithin(ctx, receiver, sb.ClampMaxMessages(inputs), sb.ClampMaxWait(inputs))
	if err != nil {
		return sb.Fail(auth, fmt.Sprintf("Could not receive from session %s", receiver.SessionID()), err), nil
	}
	...
	if state := sb.OptionalString("session_state", inputs); state != "" {
		if err := receiver.SetSessionState(ctx, []byte(state), nil); err != nil {
			return sb.Fail(auth, "Settled the messages but could not set the session state", err), nil
		}
	}
	state, err := receiver.GetSessionState(ctx, nil)
	if err != nil {
		return sb.Fail(auth, "Received messages but could not read the session state", err), nil
	}

	out := sb.MessagesResult(sb.MessagesOutput(msgs, sb.OptionalBool("parse_json", inputs)), "Received", "session "+receiver.SessionID()+" on "+spec.Entity())
	out["session_id"] = receiver.SessionID()
	out["session_state"] = string(state)
	out["locked_until"] = receiver.LockedUntil().UTC().Format(time.RFC3339)
	return out, nil
}
