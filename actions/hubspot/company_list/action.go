...
const (
	...
	Name         = "Company: List"
	Description  = "List HubSpot companies a page at a time. Pass the returned After cursor to fetch the next page."
	Summary      = "List companies in HubSpot"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := hubspot.ListObjects(apiKey, "companies", limit, after, props)
	if err != nil {
		return hubspot.ErrorResult(err.Error()), nil
	}

	out := hubspot.ListResult(resp, "")
	out["tool_result"] = fmt.Sprintf("Listed %v companies", out["count"])
	return out, nil
}
