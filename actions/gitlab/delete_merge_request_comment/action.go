package gitlab_delete_merge_request_comment
...
var Inputs = [...]core.Connection{
	...
	{Name: "note_id", Type: core.ConnectionTypeString, Label: "Note ID", Placeholder: "The ID of the comment to delete", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	noteID, err := gitlab.RequiredString("note_id", inputs)
	if err != nil {
		return nil, err
	}

	resp, err := gitlab.ProjectAPI(token, baseURL, projectID, "DELETE", fmt.Sprintf("/merge_requests/%s/notes/%s", iid, noteID), nil)
	if err != nil {
		return gitlab.ErrorResult(fmt.Sprintf("Failed to delete comment: %s", err)), nil
	}
	...
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Deleted comment %s from MR !%s", noteID, iid),
		"success":     true,
		"error":       "",
	}, nil
}
