package gitlab_cancel_pipeline
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := gitlab.ProjectAPI(token, baseURL, projectID, "POST", fmt.Sprintf("/pipelines/%s/cancel", pipelineID), nil)
	if err != nil {
		return gitlab.ErrorResult(fmt.Sprintf("Failed to cancel pipeline: %s", err)), nil
	}
	...
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Cancelled pipeline %s [%s]", pipelineID, pipeline.Status),
