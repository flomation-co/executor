package gitlab_add_merge_request_comment
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	body, err := gitlab.RequiredString("body", inputs)
	if err != nil {
		return nil, err
	}
	...
	resp, err := gitlab.ProjectAPI(token, baseURL, projectID, "POST", fmt.Sprintf("/merge_requests/%s/notes", iid), payload)
	if err != nil {
	...
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Added comment %d to MR !%s", note.ID, iid),
