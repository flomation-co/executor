package gitlab_approve_merge_request
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := gitlab.ProjectAPI(token, baseURL, projectID, "POST", fmt.Sprintf("/merge_requests/%s/approve", iid), nil)
	if err != nil {
		return gitlab.ErrorResult(fmt.Sprintf("Failed to approve: %s", err)), nil
	}
	...
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Approved MR !%s", iid),
		"success":     true,
		"error":       "",
	}, nil
}
