package scheduling_calcom_schedule_get
...
const (
	...
	Description  = "Retrieve a single Cal.com availability schedule by its ID."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := calcom.GetResource(token, fmt.Sprintf("/schedules/%d", id), calcom.VersionSchedules, nil)
	if err != nil {
		return calcom.ErrorResult(err.Error()), nil
	}
	return calcom.ResourceResult(resp, fmt.Sprintf("Retrieved schedule %d", id)), nil
}
