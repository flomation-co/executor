package scheduling_calcom_webhook_get_all
...
const (
	...
	Name         = "Cal.com: Get Many Webhooks"
	Description  = "List the webhooks configured on your Cal.com account."
...
var Inputs = [...]core.Connection{
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Follow pagination and return every webhook"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, next, _, err := calcom.ListResources(token, "/webhooks", calcom.VersionNone, nil, calcom.ClampLimit(limit, set), skip, returnAll)
	if err != nil {
		return calcom.ErrorResult(err.Error()), nil
	}
	return calcom.ListResult(items, next, fmt.Sprintf("Retrieved %d webhooks", len(items))), nil
}
