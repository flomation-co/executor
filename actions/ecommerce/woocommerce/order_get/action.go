package ecommerce_woocommerce_order_get
...
const (
	...
	Description  = "Retrieve a single order from your WooCommerce store by ID."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := woocommerce.GetResource(auth, "/orders/"+url.PathEscape(orderID), nil)
	if err != nil {
		return woocommerce.ErrorResult(err.Error()), nil
	}
	out := woocommerce.ResourceResult(resp, fmt.Sprintf("Retrieved order %s", orderID))
	return out, nil
}
