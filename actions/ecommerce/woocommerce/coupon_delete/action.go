package ecommerce_woocommerce_coupon_delete
...
var Outputs = [...]core.Connection{
	...
	{Name: "result", Type: core.ConnectionTypeObject, Label: "Deleted Coupon"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := woocommerce.DeleteResource(auth, "/coupons/"+url.PathEscape(couponID), force)
	if err != nil {
		return woocommerce.ErrorResult(err.Error()), nil
	}
	verb := "Deleted"
	...
	out := woocommerce.ResourceResult(resp, fmt.Sprintf("%s coupon %s", verb, couponID))
	return out, nil
}
