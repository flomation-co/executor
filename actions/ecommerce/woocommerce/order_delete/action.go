package ecommerce_woocommerce_order_delete
...
const (
	...
	Description  = "Delete an order from your WooCommerce store. Deletes permanently by default; turn off Force Delete to move it to the trash instead."
...
var Outputs = [...]core.Connection{
	...
	{Name: "result", Type: core.ConnectionTypeObject, Label: "Deleted Order"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := woocommerce.DeleteResource(auth, "/orders/"+url.PathEscape(orderID), force)
	if err != nil {
		return woocommerce.ErrorResult(err.Error()), nil
	}
	verb := "Deleted"
	...
	out := woocommerce.ResourceResult(resp, fmt.Sprintf("%s order %s", verb, orderID))
	return out, nil
}
