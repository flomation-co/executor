package ecommerce_woocommerce_customer_create
...
var Inputs = [...]core.Connection{
	...
	{Name: "email", Type: core.ConnectionTypeString, Label: "Email", Placeholder: "jane@example.com", Required: true},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	email, err := woocommerce.RequiredString("email", inputs)
	if err != nil {
	...
	woocommerce.SetIfPresent(customer, inputs, "username", "username")
	...
	resp, err := woocommerce.CreateResource(auth, "/customers", customer)
	if err != nil {
		return woocommerce.ErrorResult(err.Error()), nil
	}
	out := woocommerce.ResourceResult(resp, "")
	out["tool_result"] = fmt.Sprintf("Created customer %s", out["id"])
	return out, nil
}
