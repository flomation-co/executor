package ecommerce_woocommerce_customer_delete
...
var Outputs = [...]core.Connection{
	...
	{Name: "result", Type: core.ConnectionTypeObject, Label: "Deleted Customer"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// Unlike order_delete, there is no Force Delete toggle here: WooCommerce
	// customers have no trash, so the REST API requires force=true to remove one.
	// We therefore always force-delete (passing true) and omit the toggle input.
	resp, err := woocommerce.DeleteResource(auth, "/customers/"+url.PathEscape(customerID), true)
	if err != nil {
		return woocommerce.ErrorResult(err.Error()), nil
	}
	out := woocommerce.ResourceResult(resp, fmt.Sprintf("Deleted customer %s", customerID))
	return out, nil
}
