package trello_attachment_get_all
...
const (
	...
	Description  = "List the attachments on a Trello card."
	Summary      = "List attachments in Trello"
...
var Inputs = [...]core.Connection{
	...
	{Name: "card_id", Type: core.ConnectionTypeString, Label: "Card ID", Placeholder: "The card whose attachments to fetch", Required: true},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, err := trello.GetArray(auth, "/cards/"+url.PathEscape(cardID)+"/attachments", params)
	if err != nil {
		return trello.ErrorResult(err.Error()), nil
	}
	return trello.ListResult(items, fmt.Sprintf("Retrieved %d attachment(s)", len(items))), nil
}
