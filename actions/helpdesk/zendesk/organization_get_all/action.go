package helpdesk_zendesk_organization_get_all
...
const (
	...
	Name         = "Zendesk: Get Many Organizations"
	Description  = "List all organizations in your Zendesk account. Enable Return All to auto-paginate every match."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, next, lastRaw, pages, err := zendesk.ListResources(subdomain, auth, "/organizations.json", "organizations", q, returnAll)
	if err != nil {
	...
	if returnAll && next != "" && pages >= zendesk.MaxAllPages {
		out["tool_result"] = fmt.Sprintf("Fetched %d organization(s) across %d page(s); stopped at the %d-page safety cap — pass the returned next page URL to continue", len(items), pages, zendesk.MaxAllPages)
	} else if returnAll {
		out["tool_result"] = fmt.Sprintf("Fetched all %d organization(s) across %d page(s)", len(items), pages)
	} else {
		out["tool_result"] = fmt.Sprintf("Found %d organization(s)", len(items))
	}
	return out, nil
}
