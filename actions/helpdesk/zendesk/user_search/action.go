package helpdesk_zendesk_user_search
...
var Inputs = [...]core.Connection{
	...
	{Name: "query", Type: core.ConnectionTypeString, Label: "Query", Placeholder: "Name, email, phone, or a search term"},
	{Name: "external_id", Type: core.ConnectionTypeString, Label: "External ID", Placeholder: "Match users by external ID"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	zendesk.AddFilter(q, inputs, "query", "query")
	zendesk.AddFilter(q, inputs, "external_id", "external_id")

	items, next, lastRaw, pages, err := zendesk.ListResources(subdomain, auth, "/users/search.json", "users", q, returnAll)
	if err != nil {
	...
	if returnAll && next != "" && pages >= zendesk.MaxAllPages {
		out["tool_result"] = fmt.Sprintf("Found %d user(s) across %d page(s); stopped at the %d-page safety cap — pass the returned next page URL to continue", len(items), pages, zendesk.MaxAllPages)
	} else if returnAll {
		out["tool_result"] = fmt.Sprintf("Found %d user(s) across %d page(s)", len(items), pages)
	} else {
