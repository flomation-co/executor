package helpdesk_zendesk_user_get_all
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	zendesk.AddFilter(q, inputs, "role", "role")

	items, next, lastRaw, pages, err := zendesk.ListResources(subdomain, auth, "/users.json", "users", q, returnAll)
	if err != nil {
	...
	if returnAll && next != "" && pages >= zendesk.MaxAllPages {
		out["tool_result"] = fmt.Sprintf("Fetched %d user(s) across %d page(s); stopped at the %d-page safety cap — pass the returned next page URL to continue", len(items), pages, zendesk.MaxAllPages)
	} else if returnAll {
