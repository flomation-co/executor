package helpdesk_zendesk_organization_get_related_data
...
const (
	...
	Name         = "Zendesk: Get Organization Related Data"
	Description  = "Get counts of data related to a Zendesk organization (tickets, users, etc.)."
	Summary      = "Count an organisation's tickets and users"
...
var Outputs = [...]core.Connection{
	...
	{Name: "result", Type: core.ConnectionTypeObject, Label: "Organization Related Data"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	path := "/organizations/" + url.PathEscape(id) + "/related.json"
	resp, err := zendesk.GetResource(subdomain, auth, path, url.Values{})
	if err != nil {
		return zendesk.ErrorResult(err.Error()), nil
	}
	out := zendesk.ResourceResult(resp, "organization_related", fmt.Sprintf("Retrieved related data for organization %s", id))
	return out, nil
}
