package helpdesk_intercom_team_get_all
...
const (
	...
	Name         = "Intercom: Get Many Teams"
	Description  = "List the teams in your Intercom workspace, including which admins belong to each."
	Summary      = "List your support teams"
...
var Inputs = [...]core.Connection{
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Fetch every team (ignores Limit)"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// GET /teams returns every team in one response (no pagination), so Limit
	// is applied client-side and Return All simply skips the trim.
	items, _, err := intercom.ListPage(auth, "/teams", nil, "teams")
	if err != nil {
	...
	return intercom.ListResult(items, fmt.Sprintf("Retrieved %d team(s)", len(items))), nil
}
