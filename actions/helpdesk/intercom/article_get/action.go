package helpdesk_intercom_article_get
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := intercom.GetObject(auth, "/articles/"+url.PathEscape(id), nil)
	if err != nil {
		return intercom.ErrorResult(err.Error()), nil
	}
	return intercom.ResourceResult(obj, fmt.Sprintf("Retrieved article %s", id)), nil
}
