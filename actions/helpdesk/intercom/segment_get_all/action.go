package helpdesk_intercom_segment_get_all
...
var Inputs = [...]core.Connection{
	...
	{Name: "include_count", Type: core.ConnectionTypeBoolean, Label: "Include Contact Counts", Placeholder: "Also return how many people are in each segment"},
	{Name: "limit", Type: core.ConnectionTypeString, Label: "Limit", Placeholder: "Max results (default 50)"},
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Return every segment (ignores Limit)"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	q := url.Values{}
	if v, set := intercom.OptionalBoolSet("include_count", inputs); set && v {
		q.Set("include_count", "true")
	}

	// GET /segments is unpaginated — every segment comes back in one response
	// — so Limit is applied here rather than as a per_page param.
	items, _, err := intercom.ListPage(auth, "/segments", q, "segments")
	if err != nil {
	...
	return intercom.ListResult(items, fmt.Sprintf("Retrieved %d segment(s)", len(items))), nil
}
