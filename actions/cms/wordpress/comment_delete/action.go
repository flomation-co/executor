package cms_wordpress_comment_delete
...
var Outputs = [...]core.Connection{
	...
	{Name: "result", Type: core.ConnectionTypeObject, Label: "Deleted Comment"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// WordPress trashes a comment on a plain DELETE; force=true removes it
	// permanently. Default off (trash) — the safer, WP-native behaviour.
	force := wordpress.OptionalBool("force", inputs)

	resp, err := wordpress.DeleteResource(auth, "/comments/"+url.PathEscape(commentID), force, nil)
	if err != nil {
		return wordpress.ErrorResult(err.Error()), nil
	}
	verb := "Trashed"
	...
	out := wordpress.ResourceResult(resp, fmt.Sprintf("%s comment %s", verb, commentID))
	return out, nil
}
