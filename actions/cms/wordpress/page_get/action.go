package cms_wordpress_page_get
...
var Inputs = [...]core.Connection{
	...
	{Name: "password", Type: core.ConnectionTypeString, Label: "Page Password", Placeholder: "Required to read a password-protected page"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := wordpress.GetResource(auth, "/pages/"+url.PathEscape(pageID), q)
	if err != nil {
		return wordpress.ErrorResult(err.Error()), nil
	}
	out := wordpress.ResourceResult(resp, fmt.Sprintf("Retrieved page %s", pageID))
	return out, nil
}
