package cms_wordpress_user_get_all
...
const (
	...
	Name         = "WordPress: Get Many Users"
	Description  = "List users from your WordPress site, with optional filters. Enable Return All to auto-paginate every matching user."
	Summary      = "List the users on your WordPress site"
...
var Inputs = [...]core.Connection{
	...
	{Name: "roles", Type: core.ConnectionTypeString, Label: "Roles", Placeholder: "Comma-separated role slugs to filter"},
	{
		Name:  "who",
		Type:  core.ConnectionTypeString,
		Label: "Who",
		Options: []core.ConnectionOption{
			{Name: "Any", Value: ""},
			{Name: "Authors Only", Value: "authors"},
		},
	},
	...
	{Name: "include", Type: core.ConnectionTypeString, Label: "Include IDs", Placeholder: "Comma-separated user IDs to include"},
	{Name: "exclude", Type: core.ConnectionTypeString, Label: "Exclude IDs", Placeholder: "Comma-separated user IDs to exclude"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	wordpress.AddFilter(q, inputs, "roles", "roles")
	wordpress.AddFilter(q, inputs, "who", "who")
	...
	items, total, totalPages, pages, err := wordpress.ListResources(auth, "/users", q, returnAll)
	if err != nil {
	...
	if returnAll && pages >= wordpress.MaxAllPages && totalPages > pages {
		out["tool_result"] = fmt.Sprintf("Fetched %d user(s) across %d page(s); stopped at the %d-page safety cap — narrow the filters or page through manually to get the rest", len(items), pages, wordpress.MaxAllPages)
	} else if returnAll {
		out["tool_result"] = fmt.Sprintf("Fetched all %d user(s) across %d page(s)", len(items), pages)
	} else {
		out["tool_result"] = fmt.Sprintf("Found %d user(s)", len(items))
	}
	return out, nil
}
