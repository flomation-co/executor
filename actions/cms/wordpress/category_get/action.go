package cms_wordpress_category_get
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := wordpress.GetResource(auth, "/categories/"+url.PathEscape(categoryID), q)
	if err != nil {
		return wordpress.ErrorResult(err.Error()), nil
	}
	out := wordpress.ResourceResult(resp, fmt.Sprintf("Retrieved category %s", categoryID))
	return out, nil
}
