package cms_wordpress_tag_get_all
...
const (
	...
	Name         = "WordPress: Get Many Tags"
	Description  = "List tags from your WordPress site, with optional filters. Enable Return All to auto-paginate every matching tag."
	Summary      = "List tags on your WordPress site"
...
var Inputs = [...]core.Connection{
	...
	{Name: "post", Type: core.ConnectionTypeString, Label: "Assigned to Post ID", Placeholder: "Limit to tags assigned to this post ID"},
	{Name: "slug", Type: core.ConnectionTypeString, Label: "Slug"},
	{Name: "hide_empty", Type: core.ConnectionTypeBoolean, Label: "Hide Empty (only tags with posts)"},
	{
		Name:  "order",
		Type:  core.ConnectionTypeString,
		Label: "Sort Order",
	...
	{Name: "include", Type: core.ConnectionTypeString, Label: "Include IDs", Placeholder: "Comma-separated tag IDs to include"},
	{Name: "exclude", Type: core.ConnectionTypeString, Label: "Exclude IDs", Placeholder: "Comma-separated tag IDs to exclude"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, total, totalPages, pages, err := wordpress.ListResources(auth, "/tags", q, returnAll)
	if err != nil {
	...
	if returnAll && pages >= wordpress.MaxAllPages && totalPages > pages {
		out["tool_result"] = fmt.Sprintf("Fetched %d tag(s) across %d page(s); stopped at the %d-page safety cap — narrow the filters or page through manually to get the rest", len(items), pages, wordpress.MaxAllPages)
	} else if returnAll {
		out["tool_result"] = fmt.Sprintf("Fetched all %d tag(s) across %d page(s)", len(items), pages)
	} else {
		out["tool_result"] = fmt.Sprintf("Found %d tag(s)", len(items))
	}
	return out, nil
}
