package cms_wordpress_page_delete
...
const (
	...
	Description  = "Delete a page from your WordPress site. Moves it to the Trash by default; enable Permanently Delete to remove it for good."
	Summary      = "Delete a WordPress page"
...
var Outputs = [...]core.Connection{
	...
	{Name: "result", Type: core.ConnectionTypeObject, Label: "Deleted Page"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// WordPress trashes a page on a plain DELETE; force=true removes it
	// permanently. Default off (trash) — the safer, WP-native behaviour.
	force := wordpress.OptionalBool("force", inputs)

	resp, err := wordpress.DeleteResource(auth, "/pages/"+url.PathEscape(pageID), force, nil)
	if err != nil {
		return wordpress.ErrorResult(err.Error()), nil
	}
	verb := "Trashed"
	...
	out := wordpress.ResourceResult(resp, fmt.Sprintf("%s page %s", verb, pageID))
	return out, nil
}
