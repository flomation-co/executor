package journey_calculate_route

import (
	"encoding/json"
	"fmt"

	core "flomation.app/automate/executor"
	journey "flomation.app/automate/executor/actions/journey"
)

const (
	Author       = "Andy Esser"
	Organisation = "Flomation"
	Name         = "Calculate Route"
	Description  = "Calculate a route between two locations with optional waypoints, returning distance, duration, polyline and turn-by-turn steps."
	Website      = "https://www.flomation.co"
	Icon         = "route+arrow-right"
	Date         = "11/06/2026"
	Type         = core.ActionTypeAction
)

var Inputs = [...]core.Connection{
	{
		Name:        "provider",
		Type:        core.ConnectionTypeString,
		Label:       "Map Provider",
		Placeholder: "google",
		Options: []core.ConnectionOption{
			{Name: "Google Maps", Value: "google"},
			{Name: "Mapbox", Value: "mapbox"},
		},
		Required: true,
	},
	{
		Name:        "api_key",
		Type:        core.ConnectionTypeSecret,
		Label:       "Provider API Key",
		Placeholder: "${secrets.GOOGLE_MAPS_API_KEY}",
		Required:    true,
	},
	{
		Name:        "origin",
		Type:        core.ConnectionTypeSecret,
		Label:       "Origin (address or lat,lng)",
		Placeholder: "London, UK",
		Required:    true,
	},
	{
		Name:        "destination",
		Type:        core.ConnectionTypeString,
		Label:       "Destination (address or lat,lng)",
		Placeholder: "Manchester, UK",
		Required:    true,
	},
	{
		Name:        "waypoints",
		Type:        core.ConnectionTypeString,
		Label:       "Waypoints (comma- or pipe-separated)",
		Placeholder: "Birmingham|Stoke",
	},
	{
		Name:        "mode",
		Type:        core.ConnectionTypeString,
		Label:       "Travel Mode",
		Placeholder: "driving",
		Options: []core.ConnectionOption{
			{Name: "Driving", Value: "driving"},
			{Name: "Walking", Value: "walking"},
			{Name: "Cycling", Value: "cycling"},
			{Name: "Transit", Value: "transit"},
		},
	},
	{
		Name:        "depart_at",
		Type:        core.ConnectionTypeDateTime,
		Label:       "Departure time",
		Placeholder: "leave blank for 'now'",
	},
	{
		Name:  "avoid",
		Type:  core.ConnectionTypeMultiSelect,
		Label: "Avoid",
		Options: []core.ConnectionOption{
			{Name: "Tolls", Value: "tolls"},
			{Name: "Motorways", Value: "motorways"},
			{Name: "Ferries", Value: "ferries"},
		},
	},
}

var Outputs = [...]core.Connection{
	{Name: "tool_result", Type: core.ConnectionTypeString, Label: "Result summary"},
	{Name: "distance_metres", Type: core.ConnectionTypeString, Label: "Distance (metres)"},
	{Name: "distance_miles", Type: core.ConnectionTypeString, Label: "Distance (miles)"},
	{Name: "duration_seconds", Type: core.ConnectionTypeString, Label: "Duration (seconds)"},
	{Name: "duration_friendly", Type: core.ConnectionTypeString, Label: "Duration (friendly)"},
	{Name: "polyline", Type: core.ConnectionTypeString, Label: "Encoded polyline"},
	{Name: "steps", Type: core.ConnectionTypeObject, Label: "Turn-by-turn steps"},
	{Name: "bounds", Type: core.ConnectionTypeObject, Label: "Route bounds"},
	{Name: "route", Type: core.ConnectionTypeObject, Label: "Full route JSON (for downstream actions)"},
	{Name: "success", Type: core.ConnectionTypeBoolean, Label: "Success"},
	{Name: "error", Type: core.ConnectionTypeString, Label: "Error"},
}

func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	provider, err := journey.ProviderInput(inputs)
	if err != nil {
		return nil, err
	}
	origin, err := journey.RequiredString("origin", inputs)
	if err != nil {
		return nil, err
	}
	destination, err := journey.RequiredString("destination", inputs)
	if err != nil {
		return nil, err
	}
	departAt, err := journey.OptionalTime("depart_at", inputs)
	if err != nil {
		return nil, err
	}

	params := journey.RouteParams{
		Origin:      origin,
		Destination: destination,
		Waypoints:   journey.OptionalCSV("waypoints", inputs),
		Mode:        journey.OptionalString("mode", inputs),
		DepartAt:    departAt,
		Avoid:       journey.OptionalCSV("avoid", inputs),
	}

	route, err := provider.CalculateRoute(params)
	if err != nil {
		return map[string]interface{}{
			"tool_result": fmt.Sprintf("Failed to calculate route from %s to %s: %s", origin, destination, err.Error()),
			"success":     false,
			"error":       err.Error(),
		}, err
	}

	routeJSON, _ := json.Marshal(route)

	return map[string]interface{}{
		"tool_result": fmt.Sprintf("From %s to %s: %.1f miles, %s (%d turn-by-turn step(s)).",
			origin, destination, route.DistanceMiles, route.DurationFriendly, len(route.Steps)),
		"distance_metres":   fmt.Sprintf("%.0f", route.DistanceMetres),
		"distance_miles":    fmt.Sprintf("%.2f", route.DistanceMiles),
		"duration_seconds":  fmt.Sprintf("%.0f", route.DurationSeconds),
		"duration_friendly": route.DurationFriendly,
		"polyline":          route.Polyline,
		"steps":             route.Steps,
		"bounds":            route.Bounds,
		"route":             json.RawMessage(routeJSON),
		"success":           true,
		"error":             "",
	}, nil
}
