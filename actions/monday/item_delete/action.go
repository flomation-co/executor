...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	data, err := monday.GraphQL(auth, `mutation ($itemId: ID!) { delete_item (item_id: $itemId) { id } }`,
		map[string]interface{}{"itemId": itemID})
	if err != nil {
		return monday.ErrorResult(err.Error()), nil
	}
	_ = data
	return monday.SuccessResult(itemID, nil, "Deleted item "+itemID), nil
}
