// Package list_channels lists all channels within a Microsoft Teams team.
...
var Outputs = [...]core.Connection{
	...
	{Name: "channels", Type: core.ConnectionTypeString, Label: "Channels (JSON array)"},
	{Name: "count", Type: core.ConnectionTypeString, Label: "Channel Count"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	channelsJSON, _ := json.Marshal(resp.Value)

	// Build tool_result summary
	var names []string
	for _, ch := range resp.Value {
		if name, ok := ch["displayName"].(string); ok {
			names = append(names, name)
		}
	}
	summary := fmt.Sprintf("Found %d channel(s)", len(resp.Value))
	if len(names) > 0 {
		summary += ": "
