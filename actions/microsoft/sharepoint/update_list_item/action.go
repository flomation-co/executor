// Package update_list_item updates fields on a SharePoint list item.
package update_list_item
...
var Inputs = [...]core.Connection{
	...
	{Name: "fields", Type: core.ConnectionTypeText, Label: "Fields (JSON object)", Required: true, Placeholder: `{"Status":"Completed"}`},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	reqBody, _ := json.Marshal(fieldsData)

	endpoint := fmt.Sprintf("%s/sites/%s/lists/%s/items/%s/fields",
		microsoft.GraphAPI, siteID, listID, itemID)
	...
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Updated item %s in list %s", itemID, listID),
		"success":     true,
		"error":       "",
	}, nil
}
