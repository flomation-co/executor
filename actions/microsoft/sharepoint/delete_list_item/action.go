// Package delete_list_item deletes an item from a SharePoint list.
package delete_list_item
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	endpoint := fmt.Sprintf("%s/sites/%s/lists/%s/items/%s",
	...
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Deleted item %s from list %s", itemID, listID),
		"success":     true,
		"error":       "",
	}, nil
}
