...
func processCommitments(
	...
	for i, c := range commits {
		...
		// Timing is resolved here, on the platform, in preference to
		// whatever calendar date the model wrote. The resolver reads
		// durations, relative days and calendar dates alike, and always
		// lands in the future. A model-authored due_at is only trusted
		// when the resolver cannot read the phrase at all — and never
		// when it is already in the past, which is a wrong year rather
		// than an intent to be reminded retrospectively.
		if resolved := resolveDueIn(c.DueIn); resolved != "" {
			body["due_at"] = resolved
		} else if c.DueAt != "" {
			if when, err := time.Parse(time.RFC3339, c.DueAt); err != nil {
				result.Errors = append(result.Errors,
					fmt.Sprintf("commitment[%d]: unreadable due_at %q, parked for attention", i, c.DueAt))
				body["status"] = parkedStatus
			} else if when.Before(time.Now().Add(-dueAtGrace)) {
				result.Errors = append(result.Errors,
					fmt.Sprintf("commitment[%d]: due_at %s precedes now, parked for attention", i, c.DueAt))
				body["status"] = parkedStatus
			} else {
				body["due_at"] = c.DueAt
			}
...
// parkedStatus holds back a commitment whose timing cannot be trusted.
// Nothing fires it, so a wrong date becomes a row to look at rather
// than a reminder delivered the instant it was made.
const parkedStatus = "needs_attention"

// dueAtGrace covers the seconds between an assistant's reply and the
// extraction pass that reads it, so a genuine "in 30 seconds" is not
// mistaken for a bad date.
const dueAtGrace = 5 * time.Minute

// resolveDueIn hands a human-readable time expression to the shared
// resolver in the agent category. It lives on the platform rather than
// in the model because every commitment a model dated itself on live
// was wrong and every one this resolved was right.
func resolveDueIn(s string) string {
	return agent_actions.ResolveWhenRFC3339(s, time.Now())
}
