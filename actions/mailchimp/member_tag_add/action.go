package mailchimp_member_tag_add
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	for _, n := range names {
		tags = append(tags, map[string]interface{}{"name": n, "status": "active"})
	}
	...
	return mailchimp.SuccessResult(fmt.Sprintf("Added %d tag(s) to %s", len(names), email)), nil
}
