package mailchimp_interest_category_list
...
const (
	...
	Description  = "List the interest categories (groupings) of a Mailchimp audience. Use this to discover interest category IDs."
	Summary      = "List interest categories in Mailchimp"
...
var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Interest Categories"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	path := "/lists/" + url.PathEscape(listID) + "/interest-categories"
	...
	if mailchimp.OptionalBool("return_all", inputs) {
		...
		return mailchimp.ListResult(items, total, raw, fmt.Sprintf("Found %d interest category(ies)", len(items))), nil
	}
