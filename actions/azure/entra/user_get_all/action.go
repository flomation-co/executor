package azure_entra_user_get_all
...
const (
	...
	Name         = "Entra ID: Get Many Users"
	Description  = "List Microsoft Entra ID users with raw OData $filter/$search passthrough. Advanced queries (endsWith, filter on null, $search) work — ConsistencyLevel: eventual and $count=true are sent on every request. Requires the User.Read.All application permission."
...
var Inputs = [...]core.Connection{
	...
	{Name: "filter", Type: core.ConnectionTypeString, Label: "Filter ($filter)", Placeholder: `startswith(displayName,'A') — raw OData filter; advanced operators supported`},
	{Name: "search", Type: core.ConnectionTypeString, Label: "Search ($search)", Placeholder: `"displayName:smith" — quoted property:value clauses`},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, next, err := entra.ListAll(flow, auth, "/users", q, returnAll)
	if err != nil {
		return entra.ErrorResult(err.Error()), nil
	}
	return entra.ListResult(items, entra.ListSummary("user", len(items), returnAll, next != "")), nil
}
