package azure_entra_group_list_owners
...
const (
	...
	Name         = "Entra ID: List Group Owners"
	Description  = "List a group's owners — the users allowed to manage it. Requires the Group.Read.All application permission."
...
var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Owners"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, next, err := entra.ListAll(flow, auth, "/groups/"+url.PathEscape(groupID)+"/owners", q, returnAll)
	if err != nil {
		return entra.ErrorResult(err.Error()), nil
	}
	return entra.ListResult(items, entra.ListSummary("owner", len(items), returnAll, next != "")), nil
}
