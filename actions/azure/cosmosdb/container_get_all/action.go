...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, charge, err := cosmosdb.Feed(flow, auth, http.MethodGet, cosmosdb.DBPath(db)+"/colls", "colls", cosmosdb.DBRID(db), "DocumentCollections", nil, nil, cosmosdb.ClampLimit(limit, set), returnAll)
	if err != nil {
		return cosmosdb.ErrorResult(err.Error()), nil
	}
	items = cosmosdb.SimplifyItems(items, cosmosdb.BoolDefaultTrue("simplify", inputs))
	return cosmosdb.ListResult(items, charge, fmt.Sprintf("Fetched %d containers from database %q", len(items), db)), nil
}
