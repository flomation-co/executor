...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, charge, next, err := cosmosdb.Feed(flow, auth, http.MethodGet, "/dbs", "dbs", "", "Databases", nil, nil, cosmosdb.ClampLimit(limit, set), returnAll, cosmosdb.OptionalString("continuation", inputs))
	if err != nil {
		return cosmosdb.ErrorResult(err.Error()), nil
	}
	items = cosmosdb.SimplifyItems(items, cosmosdb.BoolDefaultTrue("simplify", inputs))
	return cosmosdb.ListResult(items, charge, next, returnAll, fmt.Sprintf("Fetched %d databases", len(items))), nil
}
