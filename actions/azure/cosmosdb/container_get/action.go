// Package azure_cosmosdb_container_get reads one container's definition —
// partition key, indexing policy, TTL, unique keys.
package azure_cosmosdb_container_get
...
const (
	...
	Name         = "Cosmos DB: Get Container"
	Description  = "Retrieve a container's definition — partition key, indexing policy, TTL, and unique keys."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	return cosmosdb.ResourceResult(obj, cosmosdb.RequestCharge(resp), fmt.Sprintf("Fetched container %q", coll)), nil
}
