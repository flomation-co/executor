...
const (
	...
	Name         = "Azure Table Storage: Upsert Row"
	Description  = "Insert the row, or update it if it already exists. The action most flows want. Merge only changes the fields you supply; Replace DELETES every field you leave out"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// ParseEntity before the SDK sees anything: UpsertEntity asserts
	...
	resp, err := client.UpsertEntity(tables.Context(flow), raw, &aztables.UpsertEntityOptions{UpdateMode: mode})
	if err != nil {
	...
	return tables.ResourceResult(id, result,
		fmt.Sprintf("Upserted row %s in %s (%s)", id, table, tables.UpdateModeSummary(mode))), nil
}
