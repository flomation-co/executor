package azure_files_share_create
...
const (
	...
	Name         = "Azure Files: Create Share"
	Description  = "Create an SMB file share in the storage account, with an optional size quota and metadata. A share is the mount point — directories and files live inside it"
...
var Inputs = [...]core.Connection{
	...
	{Name: "quota", Type: core.ConnectionTypeInteger, Label: "Quota (GiB)", Placeholder: "Maximum size, 1-102400 — the account default (usually 5120) when blank"},
	{
		Name:        "access_tier",
		Type:        core.ConnectionTypeString,
		Label:       "Access Tier",
		Placeholder: "The account default when blank",
	...
	{Name: "metadata", Type: core.ConnectionTypeObject, Label: "Metadata (JSON)", Placeholder: `{"team":"ops"}`},
}
...
// Quota bounds, in GiB. The service rejects anything outside them with a bare
// 400 that names the header rather than the field.
const (
	minQuotaGiB = 1
	maxQuotaGiB = 102400
)
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if err := files.ValidateShareName(share); err != nil {
		return files.ErrorResult(err.Error()), nil
	}
	...
	if err := files.CheckResponse(resp); err != nil {
		if files.ErrorCode(resp) == "ShareAlreadyExists" {
			return files.ErrorResult(fmt.Sprintf("share %q already exists", share)), nil
		}
		return files.ErrorResult(err.Error()), nil
	}
	return files.ResourceResult(share, files.HeadersResult(share, resp.Headers), fmt.Sprintf("Created share %s", share)), nil
}
