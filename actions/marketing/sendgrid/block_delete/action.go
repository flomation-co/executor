package marketing_sendgrid_block_delete
...
var Inputs = [...]core.Connection{
	...
	{Name: "delete_all", Type: core.ConnectionTypeBoolean, Label: "Delete All", Placeholder: "Tick to clear the entire block list — cannot be combined with Emails"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	switch {
	case email != "":
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/blocks/"+url.PathEscape(email), nil, nil); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult(email, fmt.Sprintf("Deleted block for %s", email)), nil
	...
	case emails != nil:
		...
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/blocks", nil, body); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult("", fmt.Sprintf("Deleted %d block(s)", len(emails))), nil
	case deleteAll:
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/blocks", nil, map[string]interface{}{"delete_all": true}); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult("", "Deleted all blocks"), nil
	default:
