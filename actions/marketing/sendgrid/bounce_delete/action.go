package marketing_sendgrid_bounce_delete
...
var Inputs = [...]core.Connection{
	...
	{Name: "delete_all", Type: core.ConnectionTypeBoolean, Label: "Delete All", Placeholder: "Tick to clear the entire bounce list — cannot be combined with Emails"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	switch {
	case email != "":
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/bounces/"+url.PathEscape(email), nil, nil); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult(email, fmt.Sprintf("Deleted bounce for %s", email)), nil
	...
	case emails != nil:
		...
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/bounces", nil, body); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult("", fmt.Sprintf("Deleted %d bounce(s)", len(emails))), nil
	case deleteAll:
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/bounces", nil, map[string]interface{}{"delete_all": true}); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult("", "Deleted all bounces"), nil
	default:
