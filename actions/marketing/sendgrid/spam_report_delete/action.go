package marketing_sendgrid_spam_report_delete
...
const (
	...
	Name         = "SendGrid: Delete Spam Report"
	Description  = "Remove addresses from your SendGrid spam report list so mail can be sent to them again. Provide a single Email, a comma-separated Emails list, or tick Delete All to clear the entire list."
	Summary      = "Clear a spam report against an address"
...
var Inputs = [...]core.Connection{
	...
	{Name: "delete_all", Type: core.ConnectionTypeBoolean, Label: "Delete All", Placeholder: "Tick to clear the entire spam report list — cannot be combined with Emails"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	switch {
	case email != "":
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/spam_reports/"+url.PathEscape(email), nil, nil); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult(email, fmt.Sprintf("Deleted spam report for %s", email)), nil
	...
	case emails != nil:
		...
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/spam_reports", nil, body); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult("", fmt.Sprintf("Deleted %d spam report(s)", len(emails))), nil
	case deleteAll:
		if _, _, _, err := sendgrid.Do(auth, http.MethodDelete, "/v3/suppression/spam_reports", nil, map[string]interface{}{"delete_all": true}); err != nil {
			return sendgrid.ErrorResult(err.Error()), nil
		}
		return sendgrid.SuccessResult("", "Deleted all spam reports"), nil
	default:
