package marketing_sendgrid_spam_report_get
...
var Outputs = [...]core.Connection{
	...
	{Name: "result", Type: core.ConnectionTypeObject, Label: "Spam Report"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	result, _, _, err := sendgrid.Do(auth, http.MethodGet, "/v3/suppression/spam_reports/"+url.PathEscape(email), nil, nil)
	if err != nil {
		return sendgrid.ErrorResult(err.Error()), nil
	}
	// The per-email endpoint answers with a JSON array; empty means the
	// address is not on the spam report list.
	items, ok := result.([]interface{})
	if !ok {
		return sendgrid.ErrorResult("unexpected SendGrid suppression response shape"), nil
	}
	if len(items) == 0 {
		return sendgrid.ErrorResult(fmt.Sprintf("no spam report found for %s", email)), nil
	}
	obj, _ := items[0].(map[string]interface{})
	return sendgrid.ResourceResult(email, obj, fmt.Sprintf("Retrieved spam report for %s", email)), nil
}
