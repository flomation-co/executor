package marketing_sendgrid_global_unsubscribe_delete
...
var Inputs = [...]core.Connection{
	...
	{Name: "email", Type: core.ConnectionTypeString, Label: "Email", Placeholder: "recipient@example.com — the address to remove from the global unsubscribe list", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	_, _, _, err = sendgrid.Do(auth, http.MethodDelete, "/v3/asm/suppressions/global/"+url.PathEscape(email), nil, nil)
	if err != nil {
		return sendgrid.ErrorResult(err.Error()), nil
	}
	return sendgrid.SuccessResult(email, fmt.Sprintf("Removed %s from the global unsubscribe list", email)), nil
}
