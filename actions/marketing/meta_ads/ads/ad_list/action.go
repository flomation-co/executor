// Package ad_list lists ads in a Meta ad account.
package ad_list
...
const (
	...
	Name         = "Ads: List"
	Description  = "List ads in a Meta ad account, with status and creative reference."
...
// Ads inherit their parent's state, so an object can be ACTIVE in its
// own right while paused in practice. Reporting only `status` makes a paused
// account look live.
const defaultFields = "id,name,adset_id,campaign_id,status,effective_status,creative,created_time,updated_time"
...
var Outputs = [...]core.Connection{
	...
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Ads"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := meta.NewClient(token, secret).Get(flow, meta.AccountPath(account)+"/ads", p)
	if err != nil {
		return meta.ErrorResult(err.Error()), nil
	}
	...
	summary := fmt.Sprintf("Found %d ad(s)", len(items))
	if next != "" {
