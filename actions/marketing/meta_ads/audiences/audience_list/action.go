// Package audience_list lists the custom audiences in a Meta ad account.
package audience_list
...
const (
	...
	Name         = "Audiences: List"
	Description  = "List the custom audiences in a Meta ad account, with size and delivery status."
	Summary      = "List audiences in Meta Ads"
...
// approximate_count_lower_bound / delivery_status matter more than they look:
// an audience below Meta's minimum size cannot be targeted at all, and the only
// signal is the delivery status.
const defaultFields = "id,name,description,subtype,approximate_count_lower_bound,approximate_count_upper_bound,operation_status,delivery_status,time_updated"
...
var Outputs = [...]core.Connection{
	...
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Custom Audiences"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := meta.NewClient(token, secret).Get(flow, meta.AccountPath(account)+"/customaudiences", p)
	if err != nil {
		return meta.ErrorResult(err.Error()), nil
	}
	...
	summary := fmt.Sprintf("Found %d custom audience(s)", len(items))
	if next != "" {
