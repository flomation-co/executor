// Package adset_list lists ad sets in a Meta ad account.
package adset_list
...
const (
	...
	Name         = "Ad Sets: List"
	Description  = "List ad sets in a Meta ad account, with targeting, budget and schedule."
	Summary      = "List ad sets in Meta Ads"
...
// Ad sets inherit their parent's state, so an object can be ACTIVE in its
// own right while paused in practice. Reporting only `status` makes a paused
// account look live.
const defaultFields = "id,name,campaign_id,status,effective_status,daily_budget,lifetime_budget,billing_event,optimization_goal,bid_amount,start_time,end_time,targeting"
...
var Outputs = [...]core.Connection{
	...
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Ad Sets"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := meta.NewClient(token, secret).Get(flow, meta.AccountPath(account)+"/adsets", p)
	if err != nil {
		return meta.ErrorResult(err.Error()), nil
	}
	...
	summary := fmt.Sprintf("Found %d ad set(s)", len(items))
	if next != "" {
