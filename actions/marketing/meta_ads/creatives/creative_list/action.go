// Package creative_list lists the ad creatives in a Meta ad account.
package creative_list
...
const (
	...
	Name         = "Creatives: List"
	Description  = "List the ad creatives in a Meta ad account."
	Summary      = "List creatives in Meta Ads"
...
const defaultFields = "id,name,status,object_story_spec,image_hash,thumbnail_url"
...
var Outputs = [...]core.Connection{
	...
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Creatives"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := meta.NewClient(token, secret).Get(flow, meta.AccountPath(account)+"/adcreatives", p)
	if err != nil {
		return meta.ErrorResult(err.Error()), nil
	}
	...
	summary := fmt.Sprintf("Found %d creative(s)", len(items))
	if next != "" {
