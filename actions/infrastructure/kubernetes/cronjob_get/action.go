// Package infrastructure_kubernetes_cronjob_get fetches one CronJob (batch/v1)
// by name, returning its full object — schedule, suspend flag, jobTemplate, and
// status (last/next schedule times, active jobs).
package infrastructure_kubernetes_cronjob_get
...
const (
	...
	Name         = "Kubernetes Get CronJob"
	Description  = "Fetch a single CronJob by name — its schedule, suspend state, job template, and last/next run times."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "CronJob", Placeholder: "The cronjob to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.CronJobs, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched cronjob %s", name)), nil
}
