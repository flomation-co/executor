// Package infrastructure_kubernetes_namespace_list lists the namespaces in a
// cluster.
//
// Namespace is cluster-scoped, so — unlike the deployment/pod list actions —
// there is no namespace input to scope by, and the underlying List call is
// passed "" for its namespace argument.
package infrastructure_kubernetes_namespace_list
...
const (
	...
	Description  = "List the namespaces in the cluster, optionally filtered by label or field."
...
var Inputs = [...]core.Connection{
	...
	{Name: "label_selector", Type: core.ConnectionTypeString, Label: "Label Selector", Placeholder: "kubernetes.io/metadata.name=production"},
	{Name: "field_selector", Type: core.ConnectionTypeString, Label: "Field Selector", Placeholder: "status.phase=Active"},
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Follow continue tokens until every namespace is fetched"},
}

var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Namespaces"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// Namespaces are cluster-scoped, so the namespace argument to List is "".
	items, continueToken, pages, err := kubernetes.List(ctx, auth, kubernetes.Namespaces, "", opts)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	summary := fmt.Sprintf("Found %d namespace(s)", len(items))
	if opts.ReturnAll && pages >= kubernetes.MaxAllPages && continueToken != "" {
		summary = fmt.Sprintf("Fetched %d namespace(s) across %d page(s); stopped at the %d-page safety cap — "+
		...
		summary = fmt.Sprintf("Found %d namespace(s); more remain — pass the continue token back in to fetch the next page", len(items))
	}
