// Package infrastructure_kubernetes_hpa_get reads a single
// HorizontalPodAutoscaler, including its current/desired replica counts and the
// live metric readings that drive them.
//
// The HPA is read from autoscaling/v2, which is the GA version from Kubernetes
...
// node does not address, and the read would 404.
package infrastructure_kubernetes_hpa_get
...
const (
	...
	Name         = "Kubernetes Get Horizontal Pod Autoscaler"
	Description  = "Read one HorizontalPodAutoscaler — its target, its current and desired replica counts, and the live metrics driving it. Needs a Kubernetes 1.23+ cluster (autoscaling/v2)."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Autoscaler", Placeholder: "The HorizontalPodAutoscaler to read", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.HPAs, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Retrieved autoscaler %s in namespace %s", name, namespace)), nil
}
