// Package infrastructure_kubernetes_deployment_restart performs a rolling
// restart of a Deployment, exactly as `kubectl rollout restart` does.
//
// There is no "restart" verb in the Kubernetes API. What kubectl actually does is
// stamp a timestamp annotation onto the Deployment's *pod template*; because the
// template changed, the Deployment controller rolls out a new ReplicaSet and
// retires the old pods gracefully, honouring the configured surge and
// unavailability budget. Nothing ever reads the annotation's value.
//
...
package infrastructure_kubernetes_deployment_restart
...
const (
	...
	Description  = "Trigger a rolling restart of a Deployment — replaces its pods one batch at a time, with no downtime."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Deployment", Placeholder: "The deployment to restart", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Patch(ctx, auth, kubernetes.Deployments, namespace, name, patch, kubernetes.PatchStrategicMerge)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	out := kubernetes.ObjectResult(obj, fmt.Sprintf("Rolling restart of deployment %s triggered at %s", name, restartedAt))
