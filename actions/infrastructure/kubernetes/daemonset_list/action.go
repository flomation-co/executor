// Package infrastructure_kubernetes_daemonset_list lists DaemonSets, either in
// one namespace or across the whole cluster.
package infrastructure_kubernetes_daemonset_list
...
const (
	...
	Name         = "Kubernetes List DaemonSets"
	Description  = "List the DaemonSets in a namespace, or across every namespace, optionally filtered by label or field."
...
var Inputs = [...]core.Connection{
	...
	{Name: "label_selector", Type: core.ConnectionTypeString, Label: "Label Selector", Placeholder: "app=fluentd,tier!=cache"},
	{Name: "field_selector", Type: core.ConnectionTypeString, Label: "Field Selector", Placeholder: "metadata.name=node-exporter"},
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Follow continue tokens until every DaemonSet is fetched"},
}

var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "DaemonSets"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, continueToken, pages, err := kubernetes.List(ctx, auth, kubernetes.DaemonSets, namespace, opts)
	if err != nil {
	...
	summary := fmt.Sprintf("Found %d daemonset(s) in %s", len(items), scope)
	if opts.ReturnAll && pages >= kubernetes.MaxAllPages && continueToken != "" {
		summary = fmt.Sprintf("Fetched %d daemonset(s) across %d page(s) in %s; stopped at the %d-page safety cap — "+
		...
		summary = fmt.Sprintf("Found %d daemonset(s) in %s; more remain — pass the continue token back in to fetch the next page", len(items), scope)
	}
