// Package infrastructure_kubernetes_daemonset_get retrieves a single DaemonSet by
// name — its spec, and the status counts (desiredNumberScheduled,
// numberReady, numberAvailable) that tell you whether its pod is running on every
// node it should be.
package infrastructure_kubernetes_daemonset_get
...
const (
	...
	Name         = "Kubernetes Get DaemonSet"
	Description  = "Fetch a single DaemonSet by name, including its rollout status and the number of nodes it is scheduled and ready on."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "DaemonSet", Placeholder: "The daemonset to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.DaemonSets, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched daemonset %s in namespace %s", name, namespace)), nil
}
