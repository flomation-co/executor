// Package infrastructure_kubernetes_node_uncordon marks a Node schedulable
// again, as `kubectl uncordon` does — the inverse of node_cordon.
//
// A merge patch of spec.unschedulable=false lets the scheduler resume placing
// pods on the node. It adds nothing and evicts nothing, so it is not
// destructive.
package infrastructure_kubernetes_node_uncordon
...
const (
	...
	Description  = "Mark a Node schedulable again so the scheduler can place pods on it. Reverses Cordon Node."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Node", Placeholder: "The node to uncordon", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	return kubernetes.ObjectResult(obj, fmt.Sprintf("Uncordoned node %s — the scheduler can place pods on it again", name)), nil
}
