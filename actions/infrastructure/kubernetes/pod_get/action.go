// Package infrastructure_kubernetes_pod_get fetches a single Pod by name,
// returning its full object — spec, status, container statuses and conditions.
package infrastructure_kubernetes_pod_get
...
const (
	...
	Description  = "Fetch a single Pod by name, including its status, conditions and per-container state."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Pod", Placeholder: "The pod to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.Pods, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched pod %s in namespace %s", name, namespace)), nil
}
