// Package infrastructure_kubernetes_cronjob_list lists CronJobs (batch/v1),
// either in one namespace or across the whole cluster.
package infrastructure_kubernetes_cronjob_list
...
const (
	...
	Name         = "Kubernetes List CronJobs"
	Description  = "List the CronJobs in a namespace, or across every namespace, optionally filtered by label or field."
...
var Inputs = [...]core.Connection{
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Follow continue tokens until every CronJob is fetched"},
}

var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "CronJobs"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, continueToken, pages, err := kubernetes.List(ctx, auth, kubernetes.CronJobs, namespace, opts)
	if err != nil {
	...
	summary := fmt.Sprintf("Found %d cronjob(s) in %s", len(items), scope)
	if opts.ReturnAll && pages >= kubernetes.MaxAllPages && continueToken != "" {
		summary = fmt.Sprintf("Fetched %d cronjob(s) across %d page(s) in %s; stopped at the %d-page safety cap — "+
		...
		summary = fmt.Sprintf("Found %d cronjob(s) in %s; more remain — pass the continue token back in to fetch the next page", len(items), scope)
	}
