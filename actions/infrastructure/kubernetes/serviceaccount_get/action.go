// Package infrastructure_kubernetes_serviceaccount_get reads a single
// ServiceAccount, returning its full object — its labels and annotations, any
// imagePullSecrets, and the secrets field (empty on Kubernetes 1.24+, where a
// ServiceAccount no longer auto-mints a long-lived token Secret).
package infrastructure_kubernetes_serviceaccount_get
...
const (
	...
	Name         = "Kubernetes Get Service Account"
	Description  = "Fetch a single ServiceAccount by name — its labels, annotations, imagePullSecrets, and referenced secrets."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Service Account", Placeholder: "The ServiceAccount to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.ServiceAccounts, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched service account %s in namespace %s", name, namespace)), nil
}
