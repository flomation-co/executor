// Package infrastructure_kubernetes_job_get reads one Job (batch/v1) by name,
// returning its full object — including the status subfield that reports how many
// pods have succeeded, failed, or are still active.
package infrastructure_kubernetes_job_get
...
const (
	...
	Name         = "Kubernetes Get Job"
	Description  = "Fetch a single Job by name, including its status — how many pods have succeeded, failed, or are still running."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Job", Placeholder: "The job to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.Jobs, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched job %s from namespace %s", name, namespace)), nil
}
