// Package infrastructure_kubernetes_cronjob_resume un-pauses a CronJob by
// patching spec.suspend to false, so the controller resumes creating Jobs on
// the schedule.
//
// Resuming does NOT back-fill runs that were missed while suspended: the next
// Job fires at the next matching schedule tick, not immediately (unless the
// CronJob's startingDeadlineSeconds and the controller's missed-run accounting
// happen to schedule one right away). It is the inverse of cronjob_suspend.
package infrastructure_kubernetes_cronjob_resume
...
const (
	...
	Name         = "Kubernetes Resume CronJob"
	Description  = "Un-pause a suspended CronJob so it resumes scheduling Jobs. Missed runs are not backfilled."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "CronJob", Placeholder: "The cronjob to resume", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	return kubernetes.ObjectResult(obj, fmt.Sprintf("Resumed cronjob %s — it will schedule jobs again from the next matching time", name)), nil
}
