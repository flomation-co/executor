// Package infrastructure_kubernetes_configmap_get reads a single ConfigMap,
// returning its full object — including the data map — for downstream nodes.
package infrastructure_kubernetes_configmap_get
...
const (
	...
	Name         = "Kubernetes Get ConfigMap"
	Description  = "Fetch a single ConfigMap by name, including its data keys and values."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "ConfigMap", Placeholder: "The config map to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.ConfigMaps, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched config map %s from namespace %s", name, namespace)), nil
}
