// Package infrastructure_kubernetes_service_get retrieves a single Service —
// its type, selector, ports, and (for LoadBalancer services) the assigned
// external address under status.loadBalancer.
package infrastructure_kubernetes_service_get
...
const (
	...
	Name         = "Kubernetes Get Service"
	Description  = "Retrieve one Service by name — its type, selector, ports, and any assigned cluster or external IP."
...
var Inputs = [...]core.Connection{
	...
	{Name: "namespace", Type: core.ConnectionTypeString, Label: "Namespace", Placeholder: "The namespace the service lives in", Required: true},
	{Name: "name", Type: core.ConnectionTypeString, Label: "Service", Placeholder: "The service to retrieve", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.Services, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Retrieved service %s in namespace %s", name, namespace)), nil
}
