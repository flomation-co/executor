// Package infrastructure_kubernetes_ingress_list lists Ingresses (the
// networking.k8s.io/v1 kind), either in one namespace or across the cluster.
//
// There is deliberately no ingress_create action in this node. A useful Ingress
// is a tree of host rules, path backends, and TLS blocks whose shape a handful
// of flat inputs cannot express without lying about it; an operator who needs to
// create one should hand the full manifest to apply_manifest instead.
package infrastructure_kubernetes_ingress_list
...
const (
	...
	Name         = "Kubernetes List Ingresses"
	Description  = "List the Ingresses in a namespace, or across every namespace, optionally filtered by label or field."
...
var Inputs = [...]core.Connection{
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Follow continue tokens until every Ingress is fetched"},
}

var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Ingresses"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, continueToken, pages, err := kubernetes.List(ctx, auth, kubernetes.Ingresses, namespace, opts)
	if err != nil {
	...
	summary := fmt.Sprintf("Found %d ingress(es) in %s", len(items), scope)
	if opts.ReturnAll && pages >= kubernetes.MaxAllPages && continueToken != "" {
		summary = fmt.Sprintf("Fetched %d ingress(es) across %d page(s) in %s; stopped at the %d-page safety cap — "+
		...
		summary = fmt.Sprintf("Found %d ingress(es) in %s; more remain — pass the continue token back in to fetch the next page", len(items), scope)
	}
