// Package infrastructure_kubernetes_deployment_get reads a single Deployment,
// returning its full object (spec, status and all) so a flow can inspect its
// replica counts, image, conditions, or any other field.
package infrastructure_kubernetes_deployment_get
...
const (
	...
	Description  = "Fetch a single Deployment by name — its spec, replica counts, image, and rollout status."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Deployment", Placeholder: "The deployment to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched deployment %s in namespace %s", name, namespace)), nil
}
