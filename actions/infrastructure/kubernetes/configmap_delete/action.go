// Package infrastructure_kubernetes_configmap_delete permanently deletes a
// ConfigMap.
//
// Deleting a ConfigMap is destructive and not recoverable — any pod that mounts
// it or reads it as an env source will fail to start (or lose the values on its
// next restart) — so it is gated on confirm_destructive, which fails closed. The
// guard is a boolean, so a flow can bind it to a variable (e.g. ${var.approved})
// and decide at run time rather than by a box ticked at design time.
package infrastructure_kubernetes_configmap_delete
...
const (
	...
	Name         = "Kubernetes Delete ConfigMap"
	Description  = "Permanently delete a ConfigMap from a namespace. Requires explicit confirmation."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "ConfigMap", Placeholder: "The config map to delete", Required: true},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if err := kubernetes.ConfirmDestructive(inputs, "delete config map "+name+" in namespace "+namespace); err != nil {
	...
	obj, err := kubernetes.Delete(ctx, auth, kubernetes.ConfigMaps, namespace, name, kubernetes.DeleteOptions{})
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	out := kubernetes.ObjectResult(obj, fmt.Sprintf("Deleted config map %s from namespace %s", name, namespace))
