// Package infrastructure_kubernetes_ingress_get retrieves a single Ingress (the
// networking.k8s.io/v1 kind) — its rules, backends, TLS blocks, and the
// load-balancer address the controller has assigned under status.
package infrastructure_kubernetes_ingress_get
...
const (
	...
	Name         = "Kubernetes Get Ingress"
	Description  = "Retrieve one Ingress by name — its host rules, path backends, TLS blocks, and assigned load-balancer address."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Ingress", Placeholder: "The ingress to retrieve", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.Ingresses, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Retrieved ingress %s in namespace %s", name, namespace)), nil
}
