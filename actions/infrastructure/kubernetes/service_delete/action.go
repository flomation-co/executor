// Package infrastructure_kubernetes_service_delete deletes a Service.
//
// Deleting a Service removes the stable virtual IP and DNS name its clients
// resolve, so anything addressing the service by name stops routing at once —
// the pods behind it are untouched, but they become unreachable through this
// name. It is therefore gated on confirm_destructive, which fails closed (see
// kubernetes.ConfirmDestructive), and the guard is a boolean so a flow can bind
// it to a run-time variable rather than a design-time checkbox.
package infrastructure_kubernetes_service_delete
...
const (
	...
	Description  = "Permanently delete a Service, removing its stable IP and DNS name. Requires explicit confirmation."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Service", Placeholder: "The service to delete", Required: true},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if err := kubernetes.ConfirmDestructive(inputs, "delete service "+name+" in namespace "+namespace); err != nil {
	...
	obj, err := kubernetes.Delete(ctx, auth, kubernetes.Services, namespace, name, kubernetes.DeleteOptions{})
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	out := kubernetes.ObjectResult(obj, fmt.Sprintf("Service %s in namespace %s deleted", name, namespace))
	// A delete can echo a Status object rather than the deleted Service, which
