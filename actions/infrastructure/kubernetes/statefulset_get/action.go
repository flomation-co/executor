// Package infrastructure_kubernetes_statefulset_get fetches a single StatefulSet
// by name, returning its full object so a downstream node can read its replica
// count, update strategy, volume claim templates, or rollout status.
package infrastructure_kubernetes_statefulset_get
...
const (
	...
	Name         = "Kubernetes Get StatefulSet"
	Description  = "Fetch a single StatefulSet by name and return its full definition and status."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "StatefulSet", Placeholder: "The statefulset to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.StatefulSets, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched statefulset %s in namespace %s", name, namespace)), nil
}
