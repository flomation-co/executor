// Package infrastructure_kubernetes_configmap_list lists ConfigMaps, either in
// one namespace or across the whole cluster.
package infrastructure_kubernetes_configmap_list
...
const (
	...
	Name         = "Kubernetes List ConfigMaps"
	Description  = "List the ConfigMaps in a namespace, or across every namespace, optionally filtered by label or field."
...
var Inputs = [...]core.Connection{
	...
	{Name: "field_selector", Type: core.ConnectionTypeString, Label: "Field Selector", Placeholder: "metadata.name=app-config"},
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Follow continue tokens until every ConfigMap is fetched"},
}

var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "ConfigMaps"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, continueToken, pages, err := kubernetes.List(ctx, auth, kubernetes.ConfigMaps, namespace, opts)
	if err != nil {
	...
	summary := fmt.Sprintf("Found %d config map(s) in %s", len(items), scope)
	if opts.ReturnAll && pages >= kubernetes.MaxAllPages && continueToken != "" {
		summary = fmt.Sprintf("Fetched %d config map(s) across %d page(s) in %s; stopped at the %d-page safety cap — "+
		...
		summary = fmt.Sprintf("Found %d config map(s) in %s; more remain — pass the continue token back in to fetch the next page", len(items), scope)
	}
