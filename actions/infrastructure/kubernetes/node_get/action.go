// Package infrastructure_kubernetes_node_get fetches one Node by name.
//
// Node is cluster-scoped, so the object is addressed with an empty namespace.
package infrastructure_kubernetes_node_get
...
const (
	...
	Description  = "Fetch a single Node by name, including its status, capacity, conditions and labels."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Node", Placeholder: "The node to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	// Nodes are cluster-scoped, so the namespace argument to Get is "".
	obj, err := kubernetes.Get(ctx, auth, kubernetes.Nodes, "", name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched node %s", name)), nil
}
