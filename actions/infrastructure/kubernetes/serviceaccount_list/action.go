// Package infrastructure_kubernetes_serviceaccount_list lists ServiceAccounts,
// either in one namespace or across the whole cluster.
package infrastructure_kubernetes_serviceaccount_list
...
const (
	...
	Name         = "Kubernetes List Service Accounts"
	Description  = "List the ServiceAccounts in a namespace, or across every namespace, optionally filtered by label or field."
...
var Inputs = [...]core.Connection{
	...
	{Name: "field_selector", Type: core.ConnectionTypeString, Label: "Field Selector", Placeholder: "metadata.name=default"},
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Follow continue tokens until every ServiceAccount is fetched"},
}

var Outputs = [...]core.Connection{
	{Name: "results", Type: core.ConnectionTypeObject, Label: "Service Accounts"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, continueToken, pages, err := kubernetes.List(ctx, auth, kubernetes.ServiceAccounts, namespace, opts)
	if err != nil {
	...
	summary := fmt.Sprintf("Found %d service account(s) in %s", len(items), scope)
	if opts.ReturnAll && pages >= kubernetes.MaxAllPages && continueToken != "" {
		summary = fmt.Sprintf("Fetched %d service account(s) across %d page(s) in %s; stopped at the %d-page safety cap — "+
		...
		summary = fmt.Sprintf("Found %d service account(s) in %s; more remain — pass the continue token back in to fetch the next page", len(items), scope)
	}
