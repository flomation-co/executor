// Package infrastructure_kubernetes_pvc_get reads a single
// PersistentVolumeClaim, returning its full object — including its bound status,
// requested and actual capacity, StorageClass, and the PersistentVolume it is
// bound to (spec.volumeName).
package infrastructure_kubernetes_pvc_get
...
const (
	...
	Description  = "Fetch a single PersistentVolumeClaim by name — its phase, capacity, StorageClass, and bound volume."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "PVC", Placeholder: "The PersistentVolumeClaim to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	obj, err := kubernetes.Get(ctx, auth, kubernetes.PVCs, namespace, name)
	if err != nil {
		return kubernetes.ErrorResult(err.Error()), nil
	}

	return kubernetes.ObjectResult(obj, fmt.Sprintf("Fetched PVC %s in namespace %s", name, namespace)), nil
}
