// Package infrastructure_kubernetes_job_list lists Jobs (batch/v1), either in one
// namespace or across the whole cluster.
package infrastructure_kubernetes_job_list
...
const (
	...
	Name         = "Kubernetes List Jobs"
	Description  = "List the Jobs in a namespace, or across every namespace, optionally filtered by label or field."
...
var Inputs = [...]core.Connection{
	...
	{Name: "label_selector", Type: core.ConnectionTypeString, Label: "Label Selector", Placeholder: "job-name=nightly-backup,tier!=cache"},
	{Name: "field_selector", Type: core.ConnectionTypeString, Label: "Field Selector", Placeholder: "metadata.name=nightly-backup"},
	...
	{Name: "return_all", Type: core.ConnectionTypeBoolean, Label: "Return All", Placeholder: "Follow continue tokens until every Job is fetched"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, continueToken, pages, err := kubernetes.List(ctx, auth, kubernetes.Jobs, namespace, opts)
	if err != nil {
	...
	summary := fmt.Sprintf("Found %d job(s) in %s", len(items), scope)
	if opts.ReturnAll && pages >= kubernetes.MaxAllPages && continueToken != "" {
		summary = fmt.Sprintf("Fetched %d job(s) across %d page(s) in %s; stopped at the %d-page safety cap — "+
		...
		summary = fmt.Sprintf("Found %d job(s) in %s; more remain — pass the continue token back in to fetch the next page", len(items), scope)
	}
