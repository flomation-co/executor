// Package infrastructure_awx_organization_list lists the organizations on an
// AWX / AAP controller.
//
// Read-only by design. AWX's default_permissions for Organization exclude 'add',
// so only a superuser can create one — a Create Organization action would fail for
// almost every operator, and is deliberately not offered.
package infrastructure_awx_organization_list
...
const (
	...
	Name         = "AWX: List Organizations"
	Description  = "List the organizations on your AWX / AAP controller."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Name", Placeholder: "Exact organization name"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	items, total, hasMore, err := awx.List(ctx, auth, "organizations/", q, returnAll)
	if err != nil {
		return awx.ErrorResult(err.Error()), nil
	}

	summary := fmt.Sprintf("Found %d organization(s)", len(items))
	if hasMore {
