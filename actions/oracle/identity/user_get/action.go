// Package oracle_identity_user_get reads one IAM user by OCID.
package oracle_identity_user_get
...
const (
	...
	Name         = "OCI Identity: Get User"
	Description  = "Fetch a single Oracle Cloud IAM user by OCID — its name, description, email, MFA status and lifecycle state."
...
var Inputs = [...]core.Connection{
	...
	{Name: "target_user_ocid", Type: core.ConnectionTypeString, Label: "User OCID (to read)", Placeholder: "ocid1.user.oc1..aaaa… of the user to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetUser(iam.Context(), identity.GetUserRequest{UserId: &id})
	if err != nil {
		return iam.ErrorResult(auth.OCIError(err)), nil
	}
	user := iam.SummariseUser(&resp.User)
	return iam.Result(fmt.Sprintf("User %q is %s", user["name"], user["lifecycle_state"]), map[string]interface{}{"user": user, "id": user["id"]}), nil
}
