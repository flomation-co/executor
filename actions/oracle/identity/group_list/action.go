// Package oracle_identity_group_list lists the IAM groups in a compartment (the tenancy).
package oracle_identity_group_list
...
const (
	...
	Name         = "OCI Identity: List Groups"
	Description  = "List the Oracle Cloud IAM groups in a compartment (the tenancy), optionally filtered by exact name. Walks pagination up to a safe cap."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Name Filter", Placeholder: "Only the group with this exact name (optional)"},
}

var Outputs = [...]core.Connection{
	{Name: "tool_result", Type: core.ConnectionTypeString, Label: "Result summary"},
	{Name: "groups", Type: core.ConnectionTypeObject, Label: "Groups"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	for page := 0; ; page++ {
		...
		resp, err := client.ListGroups(iam.Context(), req)
		if err != nil {
			return iam.ErrorResult(auth.OCIError(err)), nil
		}
		for i := range resp.Items {
			out = append(out, iam.SummariseGroup(&resp.Items[i]))
		}
	...
	return iam.Result(fmt.Sprintf("Found %d group(s)", len(out)), map[string]interface{}{
		"groups": out, "count": fmt.Sprintf("%d", len(out)), "truncated": truncated,
	}), nil
}
