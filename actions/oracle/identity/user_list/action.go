// Package oracle_identity_user_list lists the IAM users in a compartment (the tenancy).
package oracle_identity_user_list
...
const (
	...
	Name         = "OCI Identity: List Users"
	Description  = "List the Oracle Cloud IAM users in a compartment (the tenancy), optionally filtered by exact name. Walks pagination up to a safe cap."
...
var Inputs = [...]core.Connection{
	...
	{Name: "name", Type: core.ConnectionTypeString, Label: "Name Filter", Placeholder: "Only the user with this exact name (optional)"},
}

var Outputs = [...]core.Connection{
	{Name: "tool_result", Type: core.ConnectionTypeString, Label: "Result summary"},
	{Name: "users", Type: core.ConnectionTypeObject, Label: "Users"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	for page := 0; ; page++ {
		...
		resp, err := client.ListUsers(iam.Context(), req)
		if err != nil {
			return iam.ErrorResult(auth.OCIError(err)), nil
		}
		for i := range resp.Items {
			out = append(out, iam.SummariseUser(&resp.Items[i]))
		}
	...
	return iam.Result(fmt.Sprintf("Found %d user(s)", len(out)), map[string]interface{}{
		"users": out, "count": fmt.Sprintf("%d", len(out)), "truncated": truncated,
	}), nil
}
