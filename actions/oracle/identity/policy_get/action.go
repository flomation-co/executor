// Package oracle_identity_policy_get reads one IAM policy by OCID.
package oracle_identity_policy_get
...
const (
	...
	Name         = "OCI Identity: Get Policy"
	Description  = "Fetch a single Oracle Cloud IAM policy by OCID — its name, description, compartment, statements and lifecycle state."
...
var Inputs = [...]core.Connection{
	...
	{Name: "policy_ocid", Type: core.ConnectionTypeString, Label: "Policy OCID", Placeholder: "ocid1.policy.oc1..aaaa… of the policy to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetPolicy(iam.Context(), identity.GetPolicyRequest{PolicyId: &id})
	if err != nil {
		return iam.ErrorResult(auth.OCIError(err)), nil
	}
	policy := iam.SummarisePolicy(&resp.Policy)
	return iam.Result(fmt.Sprintf("Policy %q is %s", policy["name"], policy["lifecycle_state"]), map[string]interface{}{"policy": policy, "id": policy["id"]}), nil
}
