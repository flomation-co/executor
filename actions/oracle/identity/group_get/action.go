// Package oracle_identity_group_get reads one IAM group by OCID.
package oracle_identity_group_get
...
const (
	...
	Name         = "OCI Identity: Get Group"
	Description  = "Fetch a single Oracle Cloud IAM group by OCID — its name, description, compartment and lifecycle state."
...
var Inputs = [...]core.Connection{
	...
	{Name: "group_ocid", Type: core.ConnectionTypeString, Label: "Group OCID", Placeholder: "ocid1.group.oc1..aaaa… of the group to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetGroup(iam.Context(), identity.GetGroupRequest{GroupId: &id})
	if err != nil {
		return iam.ErrorResult(auth.OCIError(err)), nil
	}
	group := iam.SummariseGroup(&resp.Group)
	return iam.Result(fmt.Sprintf("Group %q is %s", group["name"], group["lifecycle_state"]), map[string]interface{}{"group": group, "id": group["id"]}), nil
}
