// Package oracle_identity_user_delete permanently deletes an IAM user by OCID.
package oracle_identity_user_delete
...
const (
	...
	Name         = "OCI Identity: Delete User"
	Description  = "Permanently delete an Oracle Cloud IAM user by OCID. Remove the user from its groups first if OCI reports it is still a member. Synchronous."
...
var Inputs = [...]core.Connection{
	...
	{Name: "target_user_ocid", Type: core.ConnectionTypeString, Label: "User OCID (to delete)", Placeholder: "ocid1.user.oc1..aaaa… of the user to delete", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if _, err := client.DeleteUser(iam.Context(), identity.DeleteUserRequest{UserId: &id}); err != nil {
		return iam.ErrorResult(auth.OCIError(err)), nil
	}
	return iam.Result(fmt.Sprintf("Deleted user %s", id), map[string]interface{}{"id": id}), nil
}
