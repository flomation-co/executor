// Package dataflow holds what every Oracle Cloud (OCI) Data Flow action shares: the API-signing-key
// credential, the regional DataFlowClient, the resource summarisers, and the input/result helpers.
// Like the sibling OCI packages it has no Execute function, so the manifest generator skips it — but
// its category.go supplies the "Data Flow" sub-group.
//
// OCI Data Flow is a managed Apache Spark service on a single regional endpoint: define
// applications, launch and manage runs, wire private endpoints, and submit interactive statements
// against a SESSION run.
package dataflow
...
// Client is the regional Data Flow client (applications, runs, private endpoints, statements).
func Client(inputs []*coreflow.Connection) (auth Auth, client dataflow.DataFlowClient, errResult map[string]interface{}) {
	a, err := GetAuth(inputs)
	if err != nil {
		return Auth{}, dataflow.DataFlowClient{}, ErrorResult(err.Error())
	}
	c, err := dataflow.NewDataFlowClientWithConfigurationProvider(a.provider)
	if err != nil {
		return Auth{}, dataflow.DataFlowClient{}, ErrorResult(a.OCIError(err))
	}
	return a, c, nil
}
...
func SummariseApplication(r *dataflow.Application) map[string]interface{} {
	return map[string]interface{}{
		"id":              Str(r.Id),
		"display_name":    Str(r.DisplayName),
		"compartment_id":  Str(r.CompartmentId),
		"language":        string(r.Language),
		"spark_version":   Str(r.SparkVersion),
		"lifecycle_state": string(r.LifecycleState),
		"num_executors":   IntOrNil(r.NumExecutors),
		"file_uri":        Str(r.FileUri),
		"time_created":    FormatTime(r.TimeCreated),
	}
}

func SummariseApplicationSummary(r *dataflow.ApplicationSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":              Str(r.Id),
		"display_name":    Str(r.DisplayName),
		"compartment_id":  Str(r.CompartmentId),
...
func SummariseRun(r *dataflow.Run) map[string]interface{} {
	return map[string]interface{}{
		"id":                  Str(r.Id),
		"display_name":        Str(r.DisplayName),
		"compartment_id":      Str(r.CompartmentId),
		"application_id":      Str(r.ApplicationId),
		"language":            string(r.Language),
		"lifecycle_state":     string(r.LifecycleState),
		"run_duration_millis": Int64OrNil(r.RunDurationInMilliseconds),
		"time_created":        FormatTime(r.TimeCreated),
	}
}

func SummariseRunSummary(r *dataflow.RunSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":                  Str(r.Id),
		"display_name":        Str(r.DisplayName),
		"compartment_id":      Str(r.CompartmentId),
...
func SummarisePrivateEndpoint(r *dataflow.PrivateEndpoint) map[string]interface{} {
	return map[string]interface{}{
		"id":              Str(r.Id),
		"display_name":    Str(r.DisplayName),
		"compartment_id":  Str(r.CompartmentId),
		...
		"lifecycle_state": string(r.LifecycleState),
		"max_host_count":  IntOrNil(r.MaxHostCount),
		"dns_zones":       r.DnsZones,
		"time_created":    FormatTime(r.TimeCreated),
	}
}

func SummarisePrivateEndpointSummary(r *dataflow.PrivateEndpointSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":              Str(r.Id),
		"display_name":    Str(r.DisplayName),
		"compartment_id":  Str(r.CompartmentId),
...
func SummariseStatement(r *dataflow.Statement) map[string]interface{} {
	return map[string]interface{}{
		"id":              Int64OrNil(r.Id),
		"run_id":          Str(r.RunId),
		"lifecycle_state": string(r.LifecycleState),
		"progress":        FloatOrNil(r.Progress),
		"time_created":    FormatTime(r.TimeCreated),
		"time_completed":  FormatTime(r.TimeCompleted),
	}
}

func SummariseStatementSummary(r *dataflow.StatementSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":              Int64OrNil(r.Id),
		"run_id":          Str(r.RunId),
		"lifecycle_state": string(r.LifecycleState),
