// Package oracle_filestorage_snapshot_delete deletes an NFS file-system snapshot by OCID.
package oracle_filestorage_snapshot_delete
...
const (
	...
	Name         = "OCI File Storage: Delete Snapshot"
	Description  = "Permanently delete an Oracle Cloud NFS file-system snapshot by OCID. The snapshot moves to DELETING then DELETED; poll Get Snapshot (it 404s once gone)."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if _, err := client.DeleteSnapshot(fss.Context(), filestorage.DeleteSnapshotRequest{SnapshotId: &id}); err != nil {
		return fss.ErrorResult(auth.OCIError(err)), nil
	}
	return fss.Result(fmt.Sprintf("Deleted snapshot %s", id), map[string]interface{}{"id": id}), nil
}
