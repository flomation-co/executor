// Package oracle_filestorage_snapshot_get reads one file-system snapshot by OCID.
package oracle_filestorage_snapshot_get
...
const (
	...
	Name         = "OCI File Storage: Get Snapshot"
	Description  = "Fetch a single Oracle Cloud file-system snapshot by OCID — its name, parent file system, snapshot type and lifecycle state."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetSnapshot(fss.Context(), filestorage.GetSnapshotRequest{SnapshotId: &id})
	if err != nil {
		return fss.ErrorResult(auth.OCIError(err)), nil
	}
	snap := fss.SummariseSnapshot(&resp.Snapshot)
	return fss.Result(fmt.Sprintf("Snapshot %q is %s", snap["name"], snap["lifecycle_state"]), map[string]interface{}{
