// Package oracle_filestorage_file_system_delete deletes an NFS file system by OCID.
package oracle_filestorage_file_system_delete
...
const (
	...
	Name         = "OCI File Storage: Delete File System"
	Description  = "Permanently delete an Oracle Cloud NFS file system by OCID. Delete its exports and snapshots first if OCI reports it is still in use. Poll Get File System (it 404s once gone)."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if _, err := client.DeleteFileSystem(fss.Context(), filestorage.DeleteFileSystemRequest{FileSystemId: &id}); err != nil {
		return fss.ErrorResult(auth.OCIError(err)), nil
	}
	return fss.Result(fmt.Sprintf("Deleted file system %s", id), map[string]interface{}{"id": id}), nil
}
