// Package oracle_cloudguard_responder_recipe_list lists the Cloud Guard responder recipes in a
// compartment, optionally filtered by exact display name or lifecycle state. Walks pagination up
// to a safe cap.
package oracle_cloudguard_responder_recipe_list
...
const (
	...
	Name         = "OCI Cloud Guard: List Responder Recipes"
	Description  = "List the Cloud Guard responder recipes in a compartment. Optionally filter by exact display name or lifecycle state, and cap the per-page size. Walks pagination up to a safe cap."
...
var Outputs = [...]core.Connection{
	...
	{Name: "responder_recipes", Type: core.ConnectionTypeObject, Label: "Responder Recipes"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	req := cloudguard.ListResponderRecipesRequest{CompartmentId: &compartment}
	if name := cg.OptionalString("display_name", inputs); name != "" {
		req.DisplayName = &name
	}
	if state := cg.OptionalString("lifecycle_state", inputs); state != "" {
		req.LifecycleState = cloudguard.ListResponderRecipesLifecycleStateEnum(state)
	}
	...
	for page := 0; ; page++ {
		...
		resp, err := client.ListResponderRecipes(cg.Context(), req)
		if err != nil {
			return cg.ErrorResult(auth.OCIError(err)), nil
		}
		for i := range resp.Items {
			out = append(out, cg.SummariseResponderRecipeSummary(&resp.Items[i]))
		}
	...
	return cg.Result(fmt.Sprintf("Found %d responder recipe(s)", len(out)), map[string]interface{}{
		"responder_recipes": out, "count": fmt.Sprintf("%d", len(out)), "truncated": truncated,
	}), nil
}
