// Package oracle_networking_subnet_get fetches one subnet by OCID.
package oracle_networking_subnet_get
...
const (
	...
	Name         = "OCI Networking: Get Subnet"
	Description  = "Fetch one Oracle Cloud subnet by OCID."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetSubnet(net.Context(), ocicore.GetSubnetRequest{SubnetId: &id})
	if err != nil {
		return net.ErrorResult(auth.OCIError(err)), nil
	}
	subnet := net.SummariseSubnet(&resp.Subnet)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Subnet %q is %s", subnet["display_name"], subnet["lifecycle_state"]),
		"subnet":          subnet,
