// Package oracle_networking_public_ip_get fetches one public IP by OCID.
package oracle_networking_public_ip_get
...
const (
	...
	Name         = "OCI Networking: Get Public IP"
	Description  = "Fetch one Oracle Cloud public IP by OCID."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetPublicIp(net.Context(), ocicore.GetPublicIpRequest{PublicIpId: &id})
	if err != nil {
		return net.ErrorResult(auth.OCIError(err)), nil
	}
	publicIp := net.SummarisePublicIp(&resp.PublicIp)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Public IP %q is %s", publicIp["ip_address"], publicIp["lifecycle_state"]),
		"public_ip":       publicIp,
