// Package oracle_networking_service_gateway_get fetches one service gateway by OCID.
package oracle_networking_service_gateway_get
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetServiceGateway(net.Context(), ocicore.GetServiceGatewayRequest{ServiceGatewayId: &id})
	if err != nil {
		return net.ErrorResult(auth.OCIError(err)), nil
	}
	gateway := net.SummariseServiceGateway(&resp.ServiceGateway)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Service gateway %q is %s", gateway["display_name"], gateway["lifecycle_state"]),
