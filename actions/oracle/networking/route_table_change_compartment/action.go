// Package oracle_networking_route_table_change_compartment moves a route table to a
// different compartment. The move is applied synchronously.
package oracle_networking_route_table_change_compartment
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	_, err = client.ChangeRouteTableCompartment(net.Context(), ocicore.ChangeRouteTableCompartmentRequest{
		RtId:                               &id,
		ChangeRouteTableCompartmentDetails: ocicore.ChangeRouteTableCompartmentDetails{CompartmentId: &target},
	})
	if err != nil {
		return net.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Move requested for route table %s", id),
		"success":     true,
	}, nil
}
