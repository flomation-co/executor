// Package oracle_networking_subnet_change_compartment moves a subnet to a different
// compartment. Unlike the other networking operations this one is asynchronous — it
// returns a work-request id.
package oracle_networking_subnet_change_compartment
...
const (
	...
	Name         = "OCI Networking: Move Subnet to Compartment"
	Description  = "Move an Oracle Cloud subnet to a different compartment. Asynchronous — returns a work-request id."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.ChangeSubnetCompartment(net.Context(), ocicore.ChangeSubnetCompartmentRequest{
		SubnetId:                       &id,
		ChangeSubnetCompartmentDetails: ocicore.ChangeSubnetCompartmentDetails{CompartmentId: &target},
	})
	if err != nil {
		return net.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Move requested for subnet %s", id),
