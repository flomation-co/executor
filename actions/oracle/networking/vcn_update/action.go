// Package oracle_networking_vcn_update edits the mutable attributes of a
// Virtual Cloud Network (VCN) — its display name, freeform tags and ZPR-only mode.
package oracle_networking_vcn_update
...
const (
	...
	Name         = "OCI Networking: Update VCN"
	Description  = "Update editable attributes of an Oracle Cloud VCN."
...
var Inputs = [...]core.Connection{
	...
	{Name: "is_zpr_only", Type: core.ConnectionTypeBoolean, Label: "ZPR-Only Mode", Placeholder: "Enforce Zero Trust Packet Routing only (optional)"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	details := ocicore.UpdateVcnDetails{}
	if v := strings.TrimSpace(net.OptionalString("display_name", inputs)); v != "" {
		details.DisplayName = &v
	}
	if net.BoolWasSet("is_zpr_only", inputs) {
		zpr := net.OptionalBool("is_zpr_only", inputs, false)
		details.IsZprOnly = &zpr
	}
	if tags != nil {
		details.FreeformTags = tags
	}
	resp, err := client.UpdateVcn(net.Context(), ocicore.UpdateVcnRequest{VcnId: &id, UpdateVcnDetails: details})
	if err != nil {
		return net.ErrorResult(auth.OCIError(err)), nil
	}
	vcn := net.SummariseVcn(&resp.Vcn)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Updated VCN %q (%s)", vcn["display_name"], vcn["lifecycle_state"]),
		"vcn":             vcn,
