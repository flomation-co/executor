// Package oracle_networking_nsg_get fetches one Network Security Group by OCID.
package oracle_networking_nsg_get
...
const (
	...
	Name         = "OCI Networking: Get NSG"
	Description  = "Fetch one Oracle Cloud NSG by OCID."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetNetworkSecurityGroup(net.Context(), ocicore.GetNetworkSecurityGroupRequest{NetworkSecurityGroupId: &id})
	if err != nil {
		return net.ErrorResult(auth.OCIError(err)), nil
	}
	nsg := net.SummariseNsg(&resp.NetworkSecurityGroup)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("NSG %q is %s", nsg["display_name"], nsg["lifecycle_state"]),
		"nsg":             nsg,
