// Package oracle_dns_steering_policy_delete deletes a DNS steering policy by OCID.
// Steering policies are GLOBAL-only (no scope), and the delete is synchronous — a 204
// on success, with no work request to poll.
package oracle_dns_steering_policy_delete
...
const (
	...
	Name         = "OCI DNS: Delete Steering Policy"
	Description  = "Permanently delete an Oracle Cloud DNS steering policy by its OCID. Synchronous — succeeds once the policy is removed."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	req := dns.DeleteSteeringPolicyRequest{SteeringPolicyId: &id}
	if _, err := client.DeleteSteeringPolicy(dnsn.Context(), req); err != nil {
		return dnsn.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Steering policy %s deleted", id),
		"id":          id,
		"success":     true,
	}, nil
}
