// Package oracle_dns_rrset_patch adds or removes individual records within one RRSet
// (the records at a domain of a given type) without replacing the whole set — the
// surgical alternative to Update RRSet, which is a full replace.
package oracle_dns_rrset_patch
...
const (
	...
	Name         = "OCI DNS: Patch RRSet"
	Description  = "Add or remove individual records within one RRSet (the records at a domain of a given type) without replacing the whole set — the surgical alternative to Update RRSet. Each operation names an ADD/REMOVE (or REQUIRE/PROHIBIT precondition). Synchronous."
...
var Inputs = [...]core.Connection{
	...
	{Name: "items_json", Type: core.ConnectionTypeText, Label: "Operations (JSON array)", Placeholder: `[{"operation":"ADD","rdata":"10.0.0.5","ttl":300},{"operation":"REMOVE","rdata":"10.0.0.4"}] — operation is ADD/REMOVE (or REQUIRE/PROHIBIT precondition); domain/rtype default to the ones above`, Required: true},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if err := json.Unmarshal([]byte(raw), &items); err != nil {
		return dnsn.ErrorResult(fmt.Sprintf(`operations must be a JSON array of record operations, e.g. [{"operation":"ADD","rdata":"10.0.0.5","ttl":300}]: %s`, err.Error())), nil
	}
	if len(items) == 0 {
		return dnsn.ErrorResult("at least one operation is required"), nil
	}
	// Default each operation's domain/rtype to the RRSet's own so callers only supply rdata/ttl/operation.
	for i := range items {
	...
	req := dns.PatchRRSetRequest{
		ZoneNameOrId:      &zone,
		Domain:            &domain,
		Rtype:             &rtype,
		PatchRrSetDetails: dns.PatchRrSetDetails{Items: items},
	}
	if scope != "" {
		req.Scope = dns.PatchRRSetScopeEnum(scope)
	}
	resp, err := client.PatchRRSet(dnsn.Context(), req)
	if err != nil {
		return dnsn.ErrorResult(auth.OCIError(err)), nil
	}
	records := dnsn.SummariseRecords(resp.Items)
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Patched %s/%s (%d operation(s)); RRSet now has %d record(s)", domain, rtype, len(items), len(records)),
		"records":     records,
