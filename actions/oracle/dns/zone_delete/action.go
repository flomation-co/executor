// Package oracle_dns_zone_delete deletes a DNS zone by name or OCID. Asynchronous —
// returns a work-request id.
package oracle_dns_zone_delete
...
const (
	...
	Name         = "OCI DNS: Delete Zone"
	Description  = "Permanently delete an Oracle Cloud DNS zone by name or OCID. Asynchronous — returns a work-request id; poll Get Zone (it will 404 once gone)."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	req := dns.DeleteZoneRequest{ZoneNameOrId: &id}
	if scope != "" {
		req.Scope = dns.DeleteZoneScopeEnum(scope)
	}
	resp, err := client.DeleteZone(dnsn.Context(), req)
	if err != nil {
		return dnsn.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Delete requested for zone %s", id),
		"id":              id,
