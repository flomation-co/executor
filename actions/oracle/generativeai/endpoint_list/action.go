// Package oracle_generativeai_endpoint_list lists the Generative AI endpoints in a compartment,
// optionally filtered by exact display name or lifecycle state. Walks pagination up to a safe cap.
package oracle_generativeai_endpoint_list
...
const (
	...
	Name         = "OCI Generative AI: List Endpoints"
	Description  = "List the Generative AI endpoints in a compartment. Optionally filter by exact display name or lifecycle state, and cap the page size. Walks pagination up to a safe cap."
...
var Inputs = [...]core.Connection{
	...
	{Name: "lifecycle_state", Type: core.ConnectionTypeString, Label: "Lifecycle State", Placeholder: "Filter by state (optional)", Options: []core.ConnectionOption{
		{Name: "Active", Value: "ACTIVE"}, {Name: "Creating", Value: "CREATING"}, {Name: "Updating", Value: "UPDATING"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	req := generativeai.ListEndpointsRequest{CompartmentId: &compartment}
	if name := gai.OptionalString("display_name", inputs); name != "" {
		req.DisplayName = &name
	}
	if state := gai.OptionalString("lifecycle_state", inputs); state != "" {
		req.LifecycleState = generativeai.EndpointLifecycleStateEnum(state)
	}
	...
	for page := 0; ; page++ {
		...
		resp, err := client.ListEndpoints(gai.Context(), req)
		if err != nil {
			return gai.ErrorResult(auth.OCIError(err)), nil
		}
		for i := range resp.Items {
			out = append(out, gai.SummariseEndpointSummary(&resp.Items[i]))
		}
	...
	return gai.Result(fmt.Sprintf("Found %d endpoint(s)", len(out)), map[string]interface{}{
