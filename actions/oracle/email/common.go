// Package email holds what every Oracle Cloud (OCI) Email Delivery action shares: the
// API-signing-key credential, the regional EmailClient, the resource summarisers, and the
// input/result helpers. Like the sibling OCI packages it has no Execute function, so the manifest
// generator skips it — but its category.go supplies the "Email" sub-group.
//
// OCI Email Delivery is a single regional service: sending domains carry DKIM signing keys,
// approved senders assert responsibility for an envelope-from address, and a customer-level
// suppression list holds addresses that must not be mailed.
package email
...
// Client is the regional Email Delivery client.
func Client(inputs []*coreflow.Connection) (auth Auth, client email.EmailClient, errResult map[string]interface{}) {
	a, err := GetAuth(inputs)
	if err != nil {
		return Auth{}, email.EmailClient{}, ErrorResult(err.Error())
	}
	c, err := email.NewEmailClientWithConfigurationProvider(a.provider)
	if err != nil {
		return Auth{}, email.EmailClient{}, ErrorResult(a.OCIError(err))
	}
	return a, c, nil
}
...
func SummariseEmailDomain(r *email.EmailDomain) map[string]interface{} {
	return map[string]interface{}{
		"id":                         Str(r.Id),
		"name":                       Str(r.Name),
		"compartment_id":             Str(r.CompartmentId),
		"lifecycle_state":            string(r.LifecycleState),
		"active_dkim_id":             Str(r.ActiveDkimId),
		"is_spf":                     Bool(r.IsSpf),
		"domain_verification_status": string(r.DomainVerificationStatus),
		"description":                Str(r.Description),
		"time_created":               FormatTime(r.TimeCreated),
	}
}

func SummariseEmailDomainSummary(r *email.EmailDomainSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":              Str(r.Id),
		"name":            Str(r.Name),
		"compartment_id":  Str(r.CompartmentId),
...
func SummariseDkim(r *email.Dkim) map[string]interface{} {
	return map[string]interface{}{
		"id":                 Str(r.Id),
		"name":               Str(r.Name),
		"email_domain_id":    Str(r.EmailDomainId),
		"compartment_id":     Str(r.CompartmentId),
		"lifecycle_state":    string(r.LifecycleState),
		"lifecycle_details":  Str(r.LifecycleDetails),
		"description":        Str(r.Description),
		"dns_subdomain_name": Str(r.DnsSubdomainName),
		"cname_record_value": Str(r.CnameRecordValue),
		"is_imported":        Bool(r.IsImported),
		"key_length":         IntOrNil(r.KeyLength),
		"time_created":       FormatTime(r.TimeCreated),
		"time_updated":       FormatTime(r.TimeUpdated),
	}
}

func SummariseDkimSummary(r *email.DkimSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":              Str(r.Id),
		"name":            Str(r.Name),
		"email_domain_id": Str(r.EmailDomainId),
...
func SummariseSender(r *email.Sender) map[string]interface{} {
	return map[string]interface{}{
		"id":               Str(r.Id),
		...
		"lifecycle_state":  string(r.LifecycleState),
		"email_domain_id":  Str(r.EmailDomainId),
		"email_ip_pool_id": Str(r.EmailIpPoolId),
		"time_created":     FormatTime(r.TimeCreated),
	}
}

func SummariseSenderSummary(r *email.SenderSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":               Str(r.Id),
		"email_address":    Str(r.EmailAddress),
		"compartment_id":   Str(r.CompartmentId),
...
func SummariseSuppression(r *email.Suppression) map[string]interface{} {
	return map[string]interface{}{
		"id":                   Str(r.Id),
		"email_address":        Str(r.EmailAddress),
		"compartment_id":       Str(r.CompartmentId),
		"reason":               string(r.Reason),
		"message_id":           Str(r.MessageId),
		"error_detail":         Str(r.ErrorDetail),
		"error_source":         Str(r.ErrorSource),
		"time_created":         FormatTime(r.TimeCreated),
		"time_last_suppressed": FormatTime(r.TimeLastSuppressed),
	}
}

func SummariseSuppressionSummary(r *email.SuppressionSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":             Str(r.Id),
		"email_address":  Str(r.EmailAddress),
		"compartment_id": Str(r.CompartmentId),
