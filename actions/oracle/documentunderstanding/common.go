// Package documentunderstanding holds what every Oracle Cloud (OCI) Document Understanding action
// shares: the API-signing-key credential, the regional AIServiceDocumentClient, the Project / Model /
// ProcessorJob summarisers, and the input/result helpers. Like the sibling OCI packages it has no
// Execute function, so the manifest generator skips it — but its category.go supplies the
// "Document Understanding" sub-group.
//
// OCI Document Understanding (Document AI) extracts text, tables, key-values and classifications from
// documents, runs asynchronous processor jobs, and manages the projects and custom models behind them.
...
// Client is the regional AIServiceDocument client for Document Understanding.
func Client(inputs []*coreflow.Connection) (auth Auth, client aidocument.AIServiceDocumentClient, errResult map[string]interface{}) {
	a, err := GetAuth(inputs)
	if err != nil {
		return Auth{}, aidocument.AIServiceDocumentClient{}, ErrorResult(err.Error())
	}
	c, err := aidocument.NewAIServiceDocumentClientWithConfigurationProvider(a.provider)
	if err != nil {
		return Auth{}, aidocument.AIServiceDocumentClient{}, ErrorResult(a.OCIError(err))
	}
	return a, c, nil
}
...
func Float32OrNil(p *float32) interface{} {
	if p == nil {
		return nil
	}
	return *p
...
// SummariseProject condenses a full Project (get / create / update response).
func SummariseProject(r *aidocument.Project) map[string]interface{} {
...
// SummariseProjectSummary condenses a ProjectSummary (list response element).
func SummariseProjectSummary(r *aidocument.ProjectSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":                Str(r.Id),
		"display_name":      Str(r.DisplayName),
		"description":       Str(r.Description),
...
// SummariseModel condenses a full Model (get / create / update response).
func SummariseModel(r *aidocument.Model) map[string]interface{} {
	return map[string]interface{}{
		"id":                         Str(r.Id),
		"display_name":               Str(r.DisplayName),
		"description":                Str(r.Description),
		...
		"is_composed_model":          Bool(r.IsComposedModel),
		"is_quick_mode":              Bool(r.IsQuickMode),
		"max_training_time_in_hours": FloatOrNil(r.MaxTrainingTimeInHours),
		"trained_time_in_hours":      FloatOrNil(r.TrainedTimeInHours),
		"alias_name":                 Str(r.AliasName),
		"time_created":               FormatTime(r.TimeCreated),
		"time_updated":               FormatTime(r.TimeUpdated),
	}
}

// SummariseModelSummary condenses a ModelSummary (list response element).
func SummariseModelSummary(r *aidocument.ModelSummary) map[string]interface{} {
	return map[string]interface{}{
		"id":                Str(r.Id),
		"display_name":      Str(r.DisplayName),
		"description":       Str(r.Description),
		...
		"lifecycle_details": Str(r.LifecycleDetails),
		"inference_units":   IntOrNil(r.InferenceUnits),
		"precision":         Float32OrNil(r.Precision),
		"is_composed_model": Bool(r.IsComposedModel),
		"alias_name":        Str(r.AliasName),
		"time_created":      FormatTime(r.TimeCreated),
		"time_updated":      FormatTime(r.TimeUpdated),
...
// SummariseProcessorJob condenses a ProcessorJob (create / get / cancel response).
func SummariseProcessorJob(r *aidocument.ProcessorJob) map[string]interface{} {
	return map[string]interface{}{
		"id":                Str(r.Id),
		"display_name":      Str(r.DisplayName),
		"compartment_id":    Str(r.CompartmentId),
		"lifecycle_state":   string(r.LifecycleState),
		"lifecycle_details": string(r.LifecycleDetails),
		"percent_complete":  Float32OrNil(r.PercentComplete),
