// Package oracle_documentunderstanding_model_list lists the custom Document Understanding models in a
// compartment, optionally scoped to a project and filtered by display name or lifecycle state. Walks
// pagination up to a safe cap.
package oracle_documentunderstanding_model_list
...
const (
	...
	Description  = "List the custom Document Understanding models in a compartment. Optionally scope to a project and filter by exact display name or lifecycle state. Walks pagination up to a safe cap."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	req := aidocument.ListModelsRequest{CompartmentId: &compartment}
	if projectID := du.OptionalString("project_id", inputs); projectID != "" {
		req.ProjectId = &projectID
	}
	if name := du.OptionalString("display_name", inputs); name != "" {
		req.DisplayName = &name
	}
	if state := du.OptionalString("lifecycle_state", inputs); state != "" {
		req.LifecycleState = aidocument.ModelLifecycleStateEnum(state)
	}
	if raw := strings.TrimSpace(du.OptionalString("limit", inputs)); raw != "" {
		limit, convErr := strconv.Atoi(raw)
		if convErr != nil {
			return du.ErrorResult("page size must be a whole number"), nil
		}
		req.Limit = &limit
	}
	...
	truncated := false
	for page := 0; ; page++ {
		if page >= du.ListMaxPages {
			truncated = true
			break
		}
		resp, err := client.ListModels(du.Context(), req)
		if err != nil {
			return du.ErrorResult(auth.OCIError(err)), nil
		}
		for i := range resp.Items {
			out = append(out, du.SummariseModelSummary(&resp.Items[i]))
		}
	...
	return du.Result(fmt.Sprintf("Found %d model(s)", len(out)), map[string]interface{}{
