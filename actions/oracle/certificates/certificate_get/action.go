// Package oracle_certificates_certificate_get fetches a single OCI certificate by its OCID,
// returning its configuration, issuing CA, algorithms, current version and lifecycle state.
package oracle_certificates_certificate_get
...
const (
	...
	Name         = "OCI Certificates: Get Certificate"
	Description  = "Fetch a single certificate by its OCID — its configuration, issuing CA, algorithms, current version and lifecycle state."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetCertificate(certs.Context(), certificatesmanagement.GetCertificateRequest{CertificateId: &certificateID})
	if err != nil {
		return certs.ErrorResult(auth.OCIError(err)), nil
	}
	certificate := certs.SummariseCertificate(&resp.Certificate)
	return certs.Result(fmt.Sprintf("Certificate %q (%s)", certificate["name"], certificate["lifecycle_state"]), map[string]interface{}{
