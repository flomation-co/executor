// Package oracle_certificates_ca_bundle_get fetches a single OCI Certificates CA bundle by its
// OCID, returning its name, description, compartment and lifecycle state.
package oracle_certificates_ca_bundle_get
...
const (
	...
	Name         = "OCI Certificates: Get CA Bundle"
	Description  = "Fetch a single CA bundle by its OCID — its name, description, compartment and lifecycle state."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetCaBundle(certs.Context(), certificatesmanagement.GetCaBundleRequest{CaBundleId: &caBundleID})
	if err != nil {
		return certs.ErrorResult(auth.OCIError(err)), nil
	}
	bundle := certs.SummariseCaBundle(&resp.CaBundle)
	return certs.Result(fmt.Sprintf("CA bundle %q (%s)", bundle["name"], bundle["lifecycle_state"]), map[string]interface{}{
