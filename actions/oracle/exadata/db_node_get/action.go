// Package oracle_exadata_db_node_get reads one Exadata DB node by OCID — its
// hostname, DB system, fault domain and lifecycle state.
package oracle_exadata_db_node_get
...
const (
	...
	Name         = "OCI Exadata: Get DB Node"
	Description  = "Fetch a single Exadata DB node by OCID — its hostname, DB system, fault domain and lifecycle state."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetDbNode(exa.Context(), db.GetDbNodeRequest{DbNodeId: &id})
	if err != nil {
		return exa.ErrorResult(auth.OCIError(err)), nil
	}
	dbNode := exa.SummariseDbNode(&resp.DbNode)
	return exa.Result(fmt.Sprintf("DB node %q is %s", dbNode["hostname"], dbNode["lifecycle_state"]), map[string]interface{}{
		"db_node": dbNode, "id": dbNode["id"], "lifecycle_state": dbNode["lifecycle_state"],
	}), nil
}
