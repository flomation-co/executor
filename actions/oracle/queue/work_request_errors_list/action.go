// Package oracle_queue_work_request_errors_list lists the errors recorded against a Queue work
// request — the asynchronous request id returned by create/update/delete/purge operations. Use it
// to see WHY a work request failed. Walks pagination up to a safe cap.
package oracle_queue_work_request_errors_list
...
const (
	...
	Description  = "List the errors recorded against a Queue work request, so you can see why an asynchronous create, update, delete or purge failed. Walks pagination up to a safe cap."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	req := queue.ListWorkRequestErrorsRequest{WorkRequestId: &workRequestID}
	...
	for page := 0; ; page++ {
		...
		resp, err := client.ListWorkRequestErrors(q.Context(), req)
		if err != nil {
	...
	return q.Result(fmt.Sprintf("Found %d work-request error(s)", len(out)), map[string]interface{}{
