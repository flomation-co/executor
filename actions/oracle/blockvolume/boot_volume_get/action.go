// Package oracle_blockvolume_boot_volume_get reads one boot volume by OCID.
package oracle_blockvolume_boot_volume_get
...
const (
	...
	Name         = "OCI Block Volumes: Get Boot Volume"
	Description  = "Fetch a single Oracle Cloud boot volume by OCID — its size, performance tier, availability domain, lifecycle state and tags."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetBootVolume(bv.Context(), ocicore.GetBootVolumeRequest{BootVolumeId: &id})
	if err != nil {
		return bv.ErrorResult(auth.OCIError(err)), nil
	}
	vol := bv.SummariseBootVolume(&resp.BootVolume)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Boot volume %q is %s", vol["display_name"], vol["lifecycle_state"]),
		"boot_volume":     vol,
		"id":              vol["id"],
