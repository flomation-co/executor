// Package oracle_blockvolume_volume_group_get reads one volume group by OCID.
package oracle_blockvolume_volume_group_get
...
const (
	...
	Name         = "OCI Block Volumes: Get Volume Group"
	Description  = "Fetch a single Oracle Cloud volume group by OCID — its member volumes, size, availability domain and lifecycle state."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetVolumeGroup(bv.Context(), ocicore.GetVolumeGroupRequest{VolumeGroupId: &id})
	if err != nil {
		return bv.ErrorResult(auth.OCIError(err)), nil
	}
	group := bv.SummariseVolumeGroup(&resp.VolumeGroup)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Volume group %q is %s", group["display_name"], group["lifecycle_state"]),
		"volume_group":    group,
		"id":              group["id"],
