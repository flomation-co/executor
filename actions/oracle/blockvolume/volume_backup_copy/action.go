// Package oracle_blockvolume_volume_backup_copy copies a volume backup to another
// region — the cross-region DR building block. The copy runs asynchronously and
// returns a work-request id.
package oracle_blockvolume_volume_backup_copy
...
const (
	...
	Name         = "OCI Block Volumes: Copy Volume Backup"
	Description  = "Copy an Oracle Cloud volume backup to another region for cross-region DR. The copy runs asynchronously and returns a work-request id."
...
var Inputs = [...]core.Connection{
	...
	{Name: "volume_backup_ocid", Type: core.ConnectionTypeString, Label: "Volume Backup OCID", Placeholder: "ocid1.volumebackup.oc1..aaaa… (the backup to copy)", Required: true},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.CopyVolumeBackup(bv.Context(), ocicore.CopyVolumeBackupRequest{
		VolumeBackupId:          &id,
		CopyVolumeBackupDetails: details,
	})
	if err != nil {
	...
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Copying backup %s to %s", id, dest),
		"backup":          backup,
		"id":              backup["id"],
