// Package oracle_blockvolume_boot_volume_backup_delete deletes a boot-volume backup by OCID.
package oracle_blockvolume_boot_volume_backup_delete
...
const (
	...
	Name         = "OCI Block Volumes: Delete Boot Volume Backup"
	Description  = "Permanently delete an Oracle Cloud boot-volume backup by OCID. Its data cannot be recovered."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if _, err := client.DeleteBootVolumeBackup(bv.Context(), ocicore.DeleteBootVolumeBackupRequest{BootVolumeBackupId: &id}); err != nil {
		return bv.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Deleted boot volume backup %s", id),
		"id":          id,
		"success":     true,
	}, nil
}
