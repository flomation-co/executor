// Package oracle_blockvolume_boot_volume_backup_get reads one boot-volume backup by OCID.
package oracle_blockvolume_boot_volume_backup_get
...
const (
	...
	Name         = "OCI Block Volumes: Get Boot Volume Backup"
	Description  = "Fetch a single Oracle Cloud boot-volume backup by OCID — its source boot volume, size, type, lifecycle state and creation time."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetBootVolumeBackup(bv.Context(), ocicore.GetBootVolumeBackupRequest{BootVolumeBackupId: &id})
	if err != nil {
		return bv.ErrorResult(auth.OCIError(err)), nil
	}
	backup := bv.SummariseBootVolumeBackup(&resp.BootVolumeBackup)
	return map[string]interface{}{
		"tool_result":        fmt.Sprintf("Boot volume backup %q is %s", backup["display_name"], backup["lifecycle_state"]),
