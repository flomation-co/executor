// Package oracle_blockvolume_volume_group_backup_change_compartment moves a
// volume-group backup to another compartment (synchronous).
package oracle_blockvolume_volume_group_backup_change_compartment
...
const (
	...
	Name         = "OCI Block Volumes: Move Volume Group Backup to Compartment"
	Description  = "Move an Oracle Cloud volume-group backup to another compartment by OCID. The move is synchronous."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	_, err = client.ChangeVolumeGroupBackupCompartment(bv.Context(), ocicore.ChangeVolumeGroupBackupCompartmentRequest{
		VolumeGroupBackupId: &id,
		ChangeVolumeGroupBackupCompartmentDetails: ocicore.ChangeVolumeGroupBackupCompartmentDetails{
			CompartmentId: &dest,
		},
	})
	if err != nil {
	...
	return map[string]interface{}{
		"tool_result":                  fmt.Sprintf("Moved volume group backup %s to compartment %s", id, dest),
		"id":                           id,
