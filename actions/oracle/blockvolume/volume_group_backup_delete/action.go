// Package oracle_blockvolume_volume_group_backup_delete deletes a volume-group backup by OCID.
package oracle_blockvolume_volume_group_backup_delete
...
const (
	...
	Name         = "OCI Block Volumes: Delete Volume Group Backup"
	Description  = "Permanently delete an Oracle Cloud volume-group backup by OCID. Its data cannot be recovered."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if _, err := client.DeleteVolumeGroupBackup(bv.Context(), ocicore.DeleteVolumeGroupBackupRequest{VolumeGroupBackupId: &id}); err != nil {
		return bv.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Deleted volume group backup %s", id),
		"id":          id,
		"success":     true,
	}, nil
}
