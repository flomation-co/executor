// Package oracle_blockvolume_volume_get reads one block volume by OCID.
package oracle_blockvolume_volume_get
...
const (
	...
	Name         = "OCI Block Volumes: Get Volume"
	Description  = "Fetch a single Oracle Cloud block volume by OCID — its size, performance tier, availability domain, lifecycle state and tags."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetVolume(bv.Context(), ocicore.GetVolumeRequest{VolumeId: &id})
	if err != nil {
		return bv.ErrorResult(auth.OCIError(err)), nil
	}
	vol := bv.SummariseVolume(&resp.Volume)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Volume %q is %s", vol["display_name"], vol["lifecycle_state"]),
		"volume":          vol,
		"id":              vol["id"],
