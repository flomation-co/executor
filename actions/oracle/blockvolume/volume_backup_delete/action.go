// Package oracle_blockvolume_volume_backup_delete deletes a volume backup by OCID.
package oracle_blockvolume_volume_backup_delete
...
const (
	...
	Name         = "OCI Block Volumes: Delete Volume Backup"
	Description  = "Permanently delete an Oracle Cloud block volume backup by OCID. Its data cannot be recovered."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	if _, err := client.DeleteVolumeBackup(bv.Context(), ocicore.DeleteVolumeBackupRequest{VolumeBackupId: &id}); err != nil {
		return bv.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Deleted volume backup %s", id),
		"id":          id,
		"success":     true,
	}, nil
}
