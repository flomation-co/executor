// Package oracle_language_model_list lists the custom Language models in a compartment, optionally
// scoped to a project and filtered by display name or lifecycle state. Walks pagination up to a
// safe cap.
package oracle_language_model_list
...
const (
	...
	Name         = "OCI Language: List Models"
	Description  = "List the custom Language models in a compartment. Optionally scope to a project and filter by exact display name or lifecycle state. Walks pagination up to a safe cap."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	req := ailanguage.ListModelsRequest{CompartmentId: &compartment}
	...
	if state := lang.OptionalString("lifecycle_state", inputs); state != "" {
		req.LifecycleState = ailanguage.ModelLifecycleStateEnum(state)
	}
	...
	for page := 0; ; page++ {
		...
		resp, err := client.ListModels(lang.Context(), req)
		if err != nil {
			return lang.ErrorResult(auth.OCIError(err)), nil
		}
		for i := range resp.Items {
			out = append(out, lang.SummariseModelSummary(&resp.Items[i]))
		}
	...
	return lang.Result(fmt.Sprintf("Found %d model(s)", len(out)), map[string]interface{}{
