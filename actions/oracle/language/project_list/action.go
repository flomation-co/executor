// Package oracle_language_project_list lists the OCI Language projects in a compartment, optionally
// filtered by exact display name or lifecycle state. Walks pagination up to a safe cap.
package oracle_language_project_list
...
const (
	...
	Name         = "OCI Language: List Projects"
	Description  = "List the OCI Language projects in a compartment. Optionally filter by exact display name or lifecycle state, and cap the number of items. Walks pagination up to a safe cap."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	req := ailanguage.ListProjectsRequest{CompartmentId: &compartment}
	if name := lang.OptionalString("display_name", inputs); name != "" {
		req.DisplayName = &name
	}
	if state := lang.OptionalString("lifecycle_state", inputs); state != "" {
		req.LifecycleState = ailanguage.ProjectLifecycleStateEnum(state)
	}
	...
	for page := 0; ; page++ {
		...
		resp, err := client.ListProjects(lang.Context(), req)
		if err != nil {
			return lang.ErrorResult(auth.OCIError(err)), nil
		}
		for i := range resp.Items {
			out = append(out, lang.SummariseProjectSummary(&resp.Items[i]))
		}
	...
	return lang.Result(fmt.Sprintf("Found %d project(s)", len(out)), map[string]interface{}{
