// Package oracle_vault_key_backup backs up a master encryption key (via the vault's
// management endpoint) to an Object Storage bucket or a pre-authenticated URI.
package oracle_vault_key_backup
...
const (
	...
	Name         = "OCI Vault: Back Up Key"
	Description  = "Back up an Oracle Cloud master encryption key (resolved via the vault's management endpoint) to an Object Storage bucket or a pre-authenticated URI."
...
var Inputs = [...]core.Connection{
	...
	{Name: "key_ocid", Type: core.ConnectionTypeString, Label: "Key OCID", Placeholder: "ocid1.key.oc1..aaaa… to back up", Required: true},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	details := keymanagement.BackupKeyDetails{BackupLocation: loc}
	resp, err := client.BackupKey(kms.Context(), keymanagement.BackupKeyRequest{KeyId: &kid, BackupKeyDetails: details})
	if err != nil {
		return kms.ErrorResult(auth.OCIError(err)), nil
	}
	key := kms.SummariseKey(&resp.Key)
	return kms.Result(fmt.Sprintf("Key %q backup started (%s)", key["display_name"], key["lifecycle_state"]), map[string]interface{}{
