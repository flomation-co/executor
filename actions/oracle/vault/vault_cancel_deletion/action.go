// Package oracle_vault_cancel_deletion cancels a scheduled Vault deletion, returning the
// vault to its active state.
package oracle_vault_cancel_deletion
...
const (
	...
	Name         = "OCI Vault: Cancel Vault Deletion"
	Description  = "Cancel a scheduled deletion of an Oracle Cloud Vault, restoring it to its active state."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.CancelVaultDeletion(kms.Context(), keymanagement.CancelVaultDeletionRequest{VaultId: &id})
	if err != nil {
		return kms.ErrorResult(auth.OCIError(err)), nil
	}
	vault := kms.SummariseVault(&resp.Vault)
	return kms.Result(fmt.Sprintf("Cancelled deletion of vault %q — now %s", vault["display_name"], vault["lifecycle_state"]), map[string]interface{}{
