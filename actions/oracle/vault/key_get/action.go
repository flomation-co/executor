// Package oracle_vault_key_get fetches a single master key from a vault (via the
// vault's management endpoint).
package oracle_vault_key_get
...
const (
	...
	Name         = "OCI Vault: Get Key"
	Description  = "Fetch a single master encryption key from an Oracle Cloud vault by its OCID (resolved via the vault's management endpoint)."
...
var Inputs = [...]core.Connection{
	...
	{Name: "key_ocid", Type: core.ConnectionTypeString, Label: "Key OCID", Placeholder: "ocid1.key.oc1..aaaa… to fetch", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetKey(kms.Context(), keymanagement.GetKeyRequest{KeyId: &kid})
	if err != nil {
		return kms.ErrorResult(auth.OCIError(err)), nil
	}
	key := kms.SummariseKey(&resp.Key)
	return kms.Result("Fetched key "+kms.Str(resp.Key.DisplayName), map[string]interface{}{
		"key":             key,
		"id":              key["id"],
