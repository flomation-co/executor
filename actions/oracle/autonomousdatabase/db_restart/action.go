// Package oracle_autonomousdatabase_db_restart restarts an Oracle Cloud
// Autonomous Database. The operation is asynchronous — it returns immediately with
// a work-request id while the database restarts.
package oracle_autonomousdatabase_db_restart
...
const (
	...
	Name         = "OCI Autonomous Database: Restart"
	Description  = "Restart an Oracle Cloud Autonomous Database. Asynchronous — returns a work-request id while the database restarts."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.RestartAutonomousDatabase(adb.Context(), database.RestartAutonomousDatabaseRequest{AutonomousDatabaseId: &id})
	if err != nil {
		return adb.ErrorResult(auth.OCIError(err)), nil
	}
	db := adb.SummariseAutonomousDatabase(&resp.AutonomousDatabase)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Restart requested for Autonomous Database %q (now %s)", db["display_name"], db["lifecycle_state"]),
		"database":        db,
