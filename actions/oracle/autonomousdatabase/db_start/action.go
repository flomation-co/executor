// Package oracle_autonomousdatabase_db_start starts a stopped Oracle Cloud
// Autonomous Database. The operation is asynchronous — it returns immediately with
// a work-request id while the database transitions STARTING → AVAILABLE.
package oracle_autonomousdatabase_db_start
...
const (
	...
	Name         = "OCI Autonomous Database: Start"
	Description  = "Start a stopped Oracle Cloud Autonomous Database. Asynchronous — returns a work-request id while the database transitions to AVAILABLE."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.StartAutonomousDatabase(adb.Context(), database.StartAutonomousDatabaseRequest{AutonomousDatabaseId: &id})
	if err != nil {
		return adb.ErrorResult(auth.OCIError(err)), nil
	}
	db := adb.SummariseAutonomousDatabase(&resp.AutonomousDatabase)
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Start requested for Autonomous Database %q (now %s)", db["display_name"], db["lifecycle_state"]),
		"database":        db,
