// Package oracle_networkloadbalancer_network_load_balancer_delete deletes a network
// load balancer by OCID. Deletion is asynchronous — returns a work-request id.
package oracle_networkloadbalancer_network_load_balancer_delete
...
const (
	...
	Name         = "OCI Network Load Balancer: Delete Network Load Balancer"
	Description  = "Permanently delete an Oracle Cloud network load balancer by OCID. Deletion is asynchronous — returns a work-request id to poll with Get Work Request."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.DeleteNetworkLoadBalancer(nlbn.Context(), nlb.DeleteNetworkLoadBalancerRequest{NetworkLoadBalancerId: &id})
	if err != nil {
		return nlbn.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Delete requested for network load balancer %s — poll work request %s", id, nlbn.Str(resp.OpcWorkRequestId)),
		"id":              id,
