// Package oracle_networkloadbalancer_backend_set_get_health_checker reads the
// health-checker policy of one backend set of a network load balancer by name.
package oracle_networkloadbalancer_backend_set_get_health_checker
...
const (
	...
	Name         = "OCI Network Load Balancer: Get Health Checker"
	Description  = "Read the health-checker policy of one backend set of an Oracle Cloud network load balancer — its protocol, port, URL path, return code and timing."
...
var Outputs = [...]core.Connection{
	...
	{Name: "health_checker", Type: core.ConnectionTypeObject, Label: "Health Checker"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetHealthChecker(nlbn.Context(), nlb.GetHealthCheckerRequest{NetworkLoadBalancerId: &nlbID, BackendSetName: &name})
	if err != nil {
		return nlbn.ErrorResult(auth.OCIError(err)), nil
	}
	summary := nlbn.SummariseHealthChecker(&resp.HealthChecker)
	return map[string]interface{}{
		"tool_result":    fmt.Sprintf("Health checker for backend set %q (%s)", name, summary["protocol"]),
