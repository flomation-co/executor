// Package oracle_networkloadbalancer_backend_set_delete deletes one backend set of a
// network load balancer by name. Deletion is asynchronous — returns a work-request id.
package oracle_networkloadbalancer_backend_set_delete
...
const (
	...
	Name         = "OCI Network Load Balancer: Delete Backend Set"
	Description  = "Delete a backend set of an Oracle Cloud network load balancer by name. Deletion is asynchronous — returns a work-request id to poll with Get Work Request."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.DeleteBackendSet(nlbn.Context(), nlb.DeleteBackendSetRequest{NetworkLoadBalancerId: &nlbID, BackendSetName: &name})
	if err != nil {
		return nlbn.ErrorResult(auth.OCIError(err)), nil
	}
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Delete requested for backend set %q — poll work request %s", name, nlbn.Str(resp.OpcWorkRequestId)),
		"name":            name,
