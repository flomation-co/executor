// Package oracle_networkloadbalancer_backend_get reads one backend server of a
// network load balancer's backend set by name.
package oracle_networkloadbalancer_backend_get
...
const (
	...
	Name         = "OCI Network Load Balancer: Get Backend"
	Description  = "Fetch one backend server of an Oracle Cloud network load balancer's backend set by name — its target, port, weight and drain/backup/offline state."
...
var Inputs = [...]core.Connection{
	...
	{Name: "backend_name", Type: core.ConnectionTypeString, Label: "Backend Name", Placeholder: "The backend name, e.g. 10.0.0.3:8080 or the assigned name", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	setName, err := nlbn.RequiredString("backend_set_name", inputs)
	if err != nil {
	...
	resp, err := client.GetBackend(nlbn.Context(), nlb.GetBackendRequest{
	...
	summary := nlbn.SummariseBackend(&resp.Backend)
	return map[string]interface{}{
		"tool_result": fmt.Sprintf("Backend %q in backend set %q", summary["name"], setName),
		"backend":     summary,
		"name":        summary["name"],
		"success":     true,
	}, nil
