// Package oracle_networkloadbalancer_listener_delete removes a listener (identified by
// name) from a network load balancer. Deletion is asynchronous — returns a work-request
// id to poll with Get Work Request.
package oracle_networkloadbalancer_listener_delete
...
const (
	...
	Name         = "OCI Network Load Balancer: Delete Listener"
	Description  = "Remove a listener (identified by name) from an Oracle Cloud network load balancer. Deletion is asynchronous — returns a work-request id to poll with Get Work Request."
...
var Inputs = [...]core.Connection{
	...
	{Name: "listener_name", Type: core.ConnectionTypeString, Label: "Listener Name", Placeholder: "The name of the listener to delete, e.g. tcp-listener", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.DeleteListener(nlbn.Context(), nlb.DeleteListenerRequest{
	...
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Deleting listener %q from network load balancer %s — poll work request %s", name, nlbID, nlbn.Str(resp.OpcWorkRequestId)),
		"listener_name":   name,
