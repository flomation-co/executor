// Package oracle_networkloadbalancer_backend_delete removes a backend server from a
// backend set of a network load balancer. Deletion is asynchronous — returns a
// work-request id.
package oracle_networkloadbalancer_backend_delete
...
const (
	...
	Name         = "OCI Network Load Balancer: Delete Backend"
	Description  = "Remove a backend server from a backend set of an Oracle Cloud network load balancer, by backend name. Deletion is asynchronous — returns a work-request id to poll with Get Work Request."
...
var Inputs = [...]core.Connection{
	...
	{Name: "backend_name", Type: core.ConnectionTypeString, Label: "Backend Name", Placeholder: "The backend to remove — its name, or <ip>:<port> / <targetOcid>:<port>, e.g. 10.0.1.5:8080", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.DeleteBackend(nlbn.Context(), nlb.DeleteBackendRequest{
	...
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Removing backend %q from backend set %q — poll work request %s", backendName, bsName, nlbn.Str(resp.OpcWorkRequestId)),
