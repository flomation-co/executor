// Package oracle_containerengine_workload_mapping_get reads one OKE workload mapping by
// OCID — the namespace-to-compartment mapping that lets pods assume an IAM identity.
package oracle_containerengine_workload_mapping_get
...
const (
	...
	Name         = "OCI Container Engine: Get Workload Mapping"
	Description  = "Fetch a single Oracle Cloud OKE workload mapping by OCID — its namespace, mapped tenancy/compartment, and lifecycle state."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetWorkloadMapping(oke.Context(), okesdk.GetWorkloadMappingRequest{ClusterId: &clusterID, WorkloadMappingId: &wmid})
	if err != nil {
		return oke.ErrorResult(auth.OCIError(err)), nil
	}
	mapping := oke.SummariseWorkloadMapping(&resp.WorkloadMapping)
	return oke.Result(fmt.Sprintf("Workload mapping for namespace %q is %s", mapping["namespace"], mapping["lifecycle_state"]), map[string]interface{}{
