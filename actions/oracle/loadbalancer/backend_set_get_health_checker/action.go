// Package oracle_loadbalancer_backend_set_get_health_checker reads the health-check
// policy attached to one backend set of an Oracle Cloud load balancer.
package oracle_loadbalancer_backend_set_get_health_checker
...
const (
	...
	Name         = "OCI Load Balancer: Get Health Checker"
	Description  = "Read the health-checker policy attached to a backend set of an Oracle Cloud load balancer — its protocol, port, URL path, expected return code and retries."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.GetHealthChecker(lbn.Context(), lb.GetHealthCheckerRequest{LoadBalancerId: &lbID, BackendSetName: &name})
	if err != nil {
		return lbn.ErrorResult(auth.OCIError(err)), nil
	}
	summary := lbn.SummariseHealthChecker(&resp.HealthChecker)
