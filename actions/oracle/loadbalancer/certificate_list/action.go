// Package oracle_loadbalancer_certificate_list lists the SSL certificate bundles
// configured on a load balancer. Synchronous read — the response carries a bare
// []Certificate (no pagination), which we summarise inline.
package oracle_loadbalancer_certificate_list
...
const (
	...
	Name         = "OCI Load Balancer: List Certificates"
	Description  = "List the SSL certificate bundles configured on an Oracle Cloud load balancer. Synchronous read."
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.ListCertificates(lbn.Context(), lb.ListCertificatesRequest{LoadBalancerId: &lbID})
	if err != nil {
		return lbn.ErrorResult(auth.OCIError(err)), nil
	}
	out := make([]map[string]interface{}, 0, len(resp.Items))
	for i := range resp.Items {
		out = append(out, lbn.SummariseCertificate(&resp.Items[i]))
	}
	return map[string]interface{}{
		"tool_result":  fmt.Sprintf("Found %d certificate(s) on load balancer %s", len(out), lbID),
		"certificates": out,
