// Package oracle_loadbalancer_rule_set_delete deletes a rule set from a load
// balancer by name. Rule sets are keyed by name within a load balancer, so this
// takes the load balancer OCID plus the rule set name. Deletion is asynchronous —
// returns a work-request id to poll with Get Work Request.
package oracle_loadbalancer_rule_set_delete
...
const (
	...
	Name         = "OCI Load Balancer: Delete Rule Set"
	Description  = "Delete a rule set from an Oracle Cloud load balancer by name. Deletion is asynchronous — returns a work-request id to poll with Get Work Request."
...
var Inputs = [...]core.Connection{
	...
	{Name: "rule_set_name", Type: core.ConnectionTypeString, Label: "Rule Set Name", Placeholder: "e.g. example_rule_set", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	resp, err := client.DeleteRuleSet(lbn.Context(), lb.DeleteRuleSetRequest{
		LoadBalancerId: &lbID,
		RuleSetName:    &name,
	})
	if err != nil {
	...
	return map[string]interface{}{
		"tool_result":     fmt.Sprintf("Delete requested for rule set %q on load balancer %s — poll work request %s", name, lbID, lbn.Str(resp.OpcWorkRequestId)),
		"rule_set_name":   name,
