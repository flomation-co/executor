// Package form_clone clones an existing JotForm form.
package form_clone
...
var Outputs = [...]core.Connection{
	...
	{Name: "form_id", Type: core.ConnectionTypeString, Label: "New Form ID"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	raw, status, err := jotform.Do(jotform.Context(flow), http.MethodPost, "/form/"+formID+"/clone", apiKey, region, nil)
	if err != nil {
	...
	newID, _ := content["id"].(string)
	result := forms_common.ObjectResult(content, fmt.Sprintf("Cloned JotForm form %s to %s.", formID, newID))
	result["form_id"] = newID
	return result, nil
}
