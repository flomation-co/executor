...
const (
	...
	Name         = "Item: Get"
	Description  = "Fetch a QuickBooks Online item by ID. Returns the item object."
	Summary      = "Look up an item in QuickBooks Online"
...
var Inputs = [...]core.Connection{
	...
	{Name: "item_id", Type: core.ConnectionTypeString, Label: "Item ID", Placeholder: "19", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	id, err := quickbooks_common.RequiredString("item_id", inputs)
	if err != nil {
		return quickbooks_common.ErrorResult(err.Error()), nil
	}

	resp, err := quickbooks_common.GetByID(flow, auth, "item", id)
	if err != nil {
		return quickbooks_common.MapError(err), nil
	}

	obj := quickbooks_common.Entity(resp, "Item")
	return quickbooks_common.ObjectResult(quickbooks_common.IDOf(obj), obj, fmt.Sprintf("Fetched QuickBooks item %s", id)), nil
}
