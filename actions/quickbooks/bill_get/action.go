...
const (
	...
	Name         = "Bill: Get"
	Description  = "Fetch a QuickBooks Online bill by ID. Returns the bill object."
	Summary      = "Look up a bill in QuickBooks Online"
...
var Inputs = [...]core.Connection{
	...
	{Name: "bill_id", Type: core.ConnectionTypeString, Label: "Bill ID", Placeholder: "145", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	id, err := quickbooks_common.RequiredString("bill_id", inputs)
	if err != nil {
		return quickbooks_common.ErrorResult(err.Error()), nil
	}

	resp, err := quickbooks_common.GetByID(flow, auth, "bill", id)
	if err != nil {
		return quickbooks_common.MapError(err), nil
	}

	obj := quickbooks_common.Entity(resp, "Bill")
	return quickbooks_common.ObjectResult(quickbooks_common.IDOf(obj), obj, fmt.Sprintf("Fetched QuickBooks bill %s", id)), nil
}
