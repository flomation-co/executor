...
const (
	...
	Name         = "Deposit: Query"
	Description  = "Query QuickBooks Online deposits. Supply a WHERE clause or a full SQL query."
	Summary      = "Search bank deposits"
...
var Inputs = [...]core.Connection{
	...
	{Name: "query", Type: core.ConnectionTypeText, Label: "Full SQL Query (overrides WHERE)", Placeholder: "select * from Deposit"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	rows := quickbooks_common.QueryRows(resp, "Deposit")
	return quickbooks_common.ListResult(rows, fmt.Sprintf("Found %d deposits", len(rows))), nil
}
