...
var Inputs = [...]core.Connection{
	...
	{Name: "query", Type: core.ConnectionTypeString, Label: "Query (SQL)", Placeholder: "select * from Item where Type = 'Service'"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	rows := quickbooks_common.QueryRows(resp, "Item")
	return quickbooks_common.ListResult(rows, fmt.Sprintf("Found %d QuickBooks item(s)", len(rows))), nil
}
