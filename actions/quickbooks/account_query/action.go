...
var Inputs = [...]core.Connection{
	...
	{Name: "query", Type: core.ConnectionTypeString, Label: "Query (SQL)", Placeholder: "select * from Account where AccountType = 'Expense'"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	rows := quickbooks_common.QueryRows(resp, "Account")
	return quickbooks_common.ListResult(rows, fmt.Sprintf("Found %d QuickBooks account(s)", len(rows))), nil
}
