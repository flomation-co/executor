...
const (
	...
	Name         = "Deposit: Get"
	Description  = "Fetch a QuickBooks Online deposit by ID. Returns the deposit object."
	Summary      = "Look up a deposit in QuickBooks Online"
...
var Inputs = [...]core.Connection{
	...
	{Name: "deposit_id", Type: core.ConnectionTypeString, Label: "Deposit ID", Placeholder: "180", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	id, err := quickbooks_common.RequiredString("deposit_id", inputs)
	if err != nil {
		return quickbooks_common.ErrorResult(err.Error()), nil
	}

	resp, err := quickbooks_common.GetByID(flow, auth, "deposit", id)
	if err != nil {
		return quickbooks_common.MapError(err), nil
	}

	obj := quickbooks_common.Entity(resp, "Deposit")
	return quickbooks_common.ObjectResult(quickbooks_common.IDOf(obj), obj, fmt.Sprintf("Fetched QuickBooks deposit %s", id)), nil
}
