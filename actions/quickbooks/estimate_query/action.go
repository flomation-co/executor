...
const (
	...
	Name         = "Estimate: Query"
	Description  = "Query QuickBooks Online estimates with SQL-like syntax. Returns matching estimate rows."
	Summary      = "Search quotes and estimates"
...
var Inputs = [...]core.Connection{
	...
	{Name: "query", Type: core.ConnectionTypeString, Label: "Query (SQL)", Placeholder: "select * from Estimate where TxnStatus = 'Pending'"},
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	rows := quickbooks_common.QueryRows(resp, "Estimate")
	return quickbooks_common.ListResult(rows, fmt.Sprintf("Found %d QuickBooks estimate(s)", len(rows))), nil
}
