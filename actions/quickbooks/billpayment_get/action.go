...
const (
	...
	Name         = "Bill Payment: Get"
	Description  = "Fetch a QuickBooks Online bill payment by ID. Returns the bill payment object."
	Summary      = "Look up a bill payment in QuickBooks Online"
...
var Inputs = [...]core.Connection{
	...
	{Name: "billpayment_id", Type: core.ConnectionTypeString, Label: "Bill Payment ID", Placeholder: "180", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	id, err := quickbooks_common.RequiredString("billpayment_id", inputs)
	if err != nil {
		return quickbooks_common.ErrorResult(err.Error()), nil
	}

	resp, err := quickbooks_common.GetByID(flow, auth, "billpayment", id)
	if err != nil {
		return quickbooks_common.MapError(err), nil
	}

	obj := quickbooks_common.Entity(resp, "BillPayment")
	return quickbooks_common.ObjectResult(quickbooks_common.IDOf(obj), obj, fmt.Sprintf("Fetched QuickBooks bill payment %s", id)), nil
}
