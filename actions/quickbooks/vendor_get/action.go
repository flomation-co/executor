...
const (
	...
	Name         = "Vendor: Get"
	Description  = "Fetch a QuickBooks Online vendor by ID. Returns the vendor object."
	Summary      = "Look up a vendor in QuickBooks Online"
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	id, err := quickbooks_common.RequiredString("vendor_id", inputs)
	if err != nil {
		return quickbooks_common.ErrorResult(err.Error()), nil
	}

	resp, err := quickbooks_common.GetByID(flow, auth, "vendor", id)
	if err != nil {
		return quickbooks_common.MapError(err), nil
	}

	obj := quickbooks_common.Entity(resp, "Vendor")
	return quickbooks_common.ObjectResult(quickbooks_common.IDOf(obj), obj, fmt.Sprintf("Fetched QuickBooks vendor %q", id)), nil
}
