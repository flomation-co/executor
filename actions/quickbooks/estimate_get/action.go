...
const (
	...
	Name         = "Estimate: Get"
	Description  = "Fetch a QuickBooks Online estimate by ID. Returns the estimate object."
	Summary      = "Look up an estimate in QuickBooks Online"
...
var Inputs = [...]core.Connection{
	...
	{Name: "estimate_id", Type: core.ConnectionTypeString, Label: "Estimate ID", Placeholder: "145", Required: true},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	id, err := quickbooks_common.RequiredString("estimate_id", inputs)
	if err != nil {
		return quickbooks_common.ErrorResult(err.Error()), nil
	}

	resp, err := quickbooks_common.GetByID(flow, auth, "estimate", id)
	if err != nil {
		return quickbooks_common.MapError(err), nil
	}

	obj := quickbooks_common.Entity(resp, "Estimate")
	return quickbooks_common.ObjectResult(quickbooks_common.IDOf(obj), obj, fmt.Sprintf("Fetched QuickBooks estimate %s", id)), nil
}
