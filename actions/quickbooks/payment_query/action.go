...
var Inputs = [...]core.Connection{
	...
	{Name: "query", Type: core.ConnectionTypeString, Label: "Query (SQL)", Placeholder: "select * from Payment where CustomerRef = '21'"},
}
...
func Execute(flow *core.Flow, node *core.Node, inputs []*core.Connection) (map[string]interface{}, error) {
	...
	rows := quickbooks_common.QueryRows(resp, "Payment")
	return quickbooks_common.ListResult(rows, fmt.Sprintf("Found %d QuickBooks payment(s)", len(rows))), nil
}
